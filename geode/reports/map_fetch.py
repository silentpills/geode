...
def fetch_maps(
    ...
    # ── Auto zoom levels ─────────────────────────────────────────────────────
    if zoom_general is None:
        zoom_general = _auto_zoom(lat, lon, 500)
    if zoom_detail is None:
        zoom_detail = _auto_zoom(lat, lon, 3)
    if zoom_satellite is None:
        zoom_satellite = _auto_zoom(lat, lon, 1.5)

    log.info(
        f"fetch_maps: lat={lat:.5f} lon={lon:.5f}  "
    ...
    def _placeholder(label: str, error: str) -> "Image.Image":
        ...
        # Labels
        draw.text((OUTPUT_W // 2, OUTPUT_H // 2 - 40), label, fill="#444", anchor="mm")
        draw.text(
            (OUTPUT_W // 2, OUTPUT_H // 2 + 30),
            f"({lat:.4f}°, {lon:.4f}°)",
            fill="#666",
            anchor="mm",
        )
        draw.text(
            (OUTPUT_W // 2, OUTPUT_H // 2 + 55), f"[{error}]", fill="#999", anchor="mm"
        )
        return img
    ...
    try:
        ...
        results["general"] = _make_map(
            osm_url, zoom_general, marker_color, dot_radius=14
        )
        log.info("  OK")
    ...
    if navigation_kml and os.path.exists(navigation_kml):
        ...
        if kml_coords:
            log.info(
                f"KML track loaded: {len(kml_coords)} points from {navigation_kml}"
            )
    ...
    try:
        ...
        results["detail"] = _make_map(
            osm_url, zoom_detail, marker_color, dot_radius=14, extra_lines=kml_lines
        )
        log.info("  OK")
    except Exception as e:
    ...
    try:
        ...
        results["satellite"] = _make_map(
            esri_url, zoom_satellite, marker_sat_color, dot_radius=14
        )
        log.info("  OK")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch map tiles for a GeoDE station.")
