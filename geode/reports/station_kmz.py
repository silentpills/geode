...
class KmzStation:
    ...
    default_nav_path: Optional[str] = (
        None  # absolute path to station-level default route KMZ
    )
    ...
    in_project: bool = (
        True  # False when project context is given and station is not in it
    )
    rinex_plot_b64: Optional[str] = (
        None  # base64 PNG, embedded as data URI in balloon script
    )
...
def _make_warning_png(size: int = 48) -> bytes:
    ...
    try:
        ...
        cx = size // 2
        draw.rectangle(
            [(cx - 2, size // 3 + 2), (cx + 2, int(size * 0.62))], fill=(30, 20, 0, 255)
        )
        dy = int(size * 0.70)
...
def _status_chip(status: str, status_color: Optional[str]) -> str:
    ...
    if hex_c:
        ...
        return (
            f'<span style="background:{bg};color:{hex_c};font-size:10px;'
...
def _photo_grid_html(images: list) -> str:
    ...
    for i in range(0, n, 2):
        ...
        for j, (uri, cap) in enumerate(pair):
            br = (
                ""
                if (j == 1 or len(pair) == 1)
                else "border-right:0.5px solid #e0ded8;"
            )
...
def build_balloon_html(s: KmzStation) -> str:
    ...
    logo_img = (
        f'<img src="{logo_uri}" style="height:28px;width:auto;opacity:.9">'
        if logo_uri
        else ""
    )
    ...
    station_info = (
        ...
        'letter-spacing:.06em;color:#9b9a96;margin-bottom:6px">Station Info</div>'
        '<table width="100%" cellpadding="0" cellspacing="0"'
        ' style="border:0.5px solid #e0ded8;border-radius:6px;overflow:hidden">'
    ...
    if encoded_photos or mon_uri:
        imagery_section = (
            '<div style="padding:8px 14px 4px">'
            '<div style="font-size:9.5px;font-weight:700;text-transform:uppercase;'
            'letter-spacing:.06em;color:#9b9a96;margin-bottom:6px">Site imagery</div>'
        )
        if encoded_photos:
    ...
    if s.visits:
        ...
        for i, v in enumerate(s.visits):
            ...
            camp = (
                ...
                if v.campaign
                else '<span style="color:#aaa;font-size:10px;font-style:italic">No campaign</span>'
            )
            rows_html += (
                f'<tr style="background:{bg}">'
    ...
    if s.rinex_plot_b64:
        ...
        rinex_btn = (
            ...
            '<a href="#" onclick="return _showRinex()" '
            'style="display:inline-block;background:#185fa5;color:#fff;'
            "font-size:11px;font-weight:600;padding:6px 18px;border-radius:6px;"
            'text-decoration:none">View RINEX Availability</a>'
            "</div>"
        )
...
def station_data_from_db(
    cnn, network_code: str, station_code: str, media_path: Optional[str] = None
) -> KmzStation:
    """
    ...
    if media_path and not os.path.isdir(media_path):
        ...
        print(" !! Images will be omitted.", file=sys.stderr)
        media_path = None
    ...
    for vr in visit_rows:
        nav_rel = vr.get(
            "navigation_file"
        )  # Django FileField — relative path within media
        ...
        if nav_rel:
            if nav_abs:
                ...
                tried = (
                    os.path.join(media_path, nav_rel)
                    if media_path
                    else "(media_path is None)"
                )
                print(
                    f"    visit {visit_date}: nav [NOT FOUND] tried={tried!r}",
                    file=sys.stderr,
                )
        visits.append(
            KmzVisit(
    ...
    try:
        ...
        _log.getLogger(__name__).warning(
            f"StationInfo check failed for {nc}.{sc}: {_e}"
        )
    ...
    try:
        with zipfile.ZipFile(path, "r") as zf:
            ...
            kn = (
                "doc.kml"
                if "doc.kml" in names
                else next((n for n in names if n.endswith(".kml")), None)
            )
...
def _embed_nav_content(output_path: str, nav_embed: list) -> None:
    """
    ...
    for station_name, visit_date, nav_path in nav_embed:
        ...
        print(
            f"    embedded nav content from {os.path.basename(nav_path)} "
            f"into {station_name}/{visit_date}",
            file=sys.stderr,
        )
        modified = True
...
def build_kmz(
    stations: list, output_path: str, project_name: Optional[str] = None
) -> None:
    """
