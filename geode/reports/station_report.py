...
from .html_utils import escape_report_text

MAP_FETCH_AVAILABLE = find_spec("staticmap") is not None
...
class StationReport:
    ...
    instrument_history_total: int = (
        0  # total sessions in DB (may exceed len(instrument_history))
    )
    ...
    station_images: list = field(
        default_factory=list
    )  # [(path, caption), …] all station images
...
def _flag(country: str) -> str:
    ...
    try:
        ...
        html = (
            f'<img src="data:image/svg+xml;base64,{b64}" alt="{alpha2}"'
...
def _section(label: str) -> str:
    return (
        f'<div class="section-header"><div class="sq"></div><span>{label}</span></div>'
    )
...
def _instr_session_card(s: InstrumentSession, is_current: bool = False) -> str:
    hdr_bg = (
        ...
        if is_current
        else "background:#f5f5f3 !important;"
    )
    border = (
        "border:0.5px solid #b5d4f4;" if is_current else "border:0.5px solid #e0ded8;"
    )
    badge = (
...
def _visit_card(v: Visit) -> str:
    ...
    if v.obs_files:
        ...
        if extra > 0:
            obs_html += (
                f'<div class="no-data" style="margin-top:4px">…and {extra} more</div>'
            )
    else:
    ...
    if valid_photos:
        ...
        for i in range(0, len(display_photos), 3):
            ...
            while cells_html.count("<td") < 3:
                cells_html += (
                    '<td class="visit-photo-cell" style="background:#f5f5f3"></td>'
                )
...
def _beachball_b64(
    strike: float, dip: float, rake: float, facecolor: str = "k"
) -> Optional[str]:
...
def _geo_event_card(eq) -> str:
    ...
    if eq.strike:
        bb_b64 = _beachball_b64(eq.strike[0], eq.dip[0], eq.rake[0], facecolor=bb_color)
        if bb_b64:
            bb_html = (
                f'<img src="data:image/png;base64,{bb_b64}" alt="Focal mechanism">'
            )
        else:
            bb_html = (
                '<div class="geo-bb-placeholder"><span>Render<br>error</span></div>'
            )
    else:
        bb_html = (
            '<div class="geo-bb-placeholder"><span>No focal<br>mechanism</span></div>'
        )
    ...
    title_html = (
        f'<a href="{usgs_url}" target="_blank">{magnitude} &ndash; {eq.location}</a>'
    )

    # Date + epicenter + depth  (pyDate.Date stores hour/minute/second)
    try:
        dt_str = (
            f"{eq.date.year}-{eq.date.month:02d}-{eq.date.day:02d}"
    ...
    dist_chip = (
        f'<span class="chip chip-gray">{eq.distance:.0f}&thinsp;km from station</span>'
    )
...
def build_report(
    ...
    """
    s = escape_report_text(s)
    etm_caption, etm_subtitle, rinex_caption, rinex_subtitle = [
        escape_report_text(value)
        for value in (etm_caption, etm_subtitle, rinex_caption, rinex_subtitle)
    ]
    ...
    if _hex:
        ...
        _fallback = (
            "chip-green" if (s.status or "").lower() == "active" else "chip-amber"
        )
        status_chip_html = _chip(f"&#10003; {s.status}", _fallback)

    # ── Header ─────────────────────────────────────────────────────────────────
    logo_html = (
        f'<img class="geode-logo" src="{logo_uri}" alt="GeoDE">' if logo_uri else ""
    )
...
<div class="location-line">{s.location_desc}</div>"""
    ...
    def _cell(uri, cap, borders=""):
        if not uri:
            return ""
        return (
            f'<div class="cell" style="{borders}">'
    ...
    cells_html = (
        ...
        + _cell(map_det_uri, "Site detail", "border-bottom:1px solid #888;")
        + _cell(
            sat_uri,
            "Satellite view &middot; station marker visible",
  ...
  <div class="metric-card"><div class="lbl">Z (ECEF)</div><div class="val">{_fmt_ecef(s.z_ecef)}</div></div>
    ...
    span_card = _info_card(
        ...
        [
            ...
            (
                "Contacts",
                f"{len(s.contacts)} people" if s.contacts else "None on record",
                not s.contacts,
            ),
        ],
    )
    ...
    if show_instruments and s.instrument_history:
        ...
        if total > shown:
            note = (
                f'<div style="font-size:11px;color:#9b9a96;font-style:italic;'
    ...
    if show_contacts:
        if s.contacts:
            ...
            contacts_section = (
                _section("Contact Information")
                + '<div class="comment-box" style="background:#f5f5f3 !important;'
                'border-color:#d0ced8;border-left-color:#9b9a96;">'
                '<p style="color:#9b9a96;font-style:italic;">No contact information on record for this station.</p>'
                "</div>"
            )
    ...
    if show_rinex and rinex_uri:
        rinex_section = (
            ...
            + (
                f'<div class="sub-note">{rinex_subtitle}</div>'
                if rinex_subtitle
                else ""
            )
            + f'<div class="rinex-portrait-wrap"><img src="{rinex_uri}" alt="RINEX availability"></div>'
            + (
                f'<div class="img-caption">{rinex_caption}</div>'
                if rinex_caption
                else ""
            )
        )
    ...
    if s.station_images:
        ...
        if rows_html:
            station_photos_section = (
                _section(
                    f"Station Photos &nbsp;&middot;&nbsp; {len(s.station_images)} image(s)"
                )
    ...
    if show_visits:
        if s.visits:
            ...
            visit_cards = "".join(
                card + comment for card, comment in (_visit_card(v) for v in s.visits)
            )
            visits_section = (
                _section(
                    f"Visit History &nbsp;&middot;&nbsp; {n_visits} visits &nbsp;&middot;&nbsp; {year_range}"
                )
                + visit_cards
            )
        else:
            visits_section = (
                ...
                + '<div class="comment-box" style="background:#f5f5f3 !important;'
                'border-color:#d0ced8;border-left-color:#9b9a96;">'
                '<p style="color:#9b9a96;font-style:italic;">No visits on record for this station.</p>'
                "</div>"
            )
    ...
    if show_geodynamics:
        if s.geodynamic_events:
            ...
            geo_section = (
                ...
                + '<div class="comment-box" style="background:#f5f5f3 !important;'
                'border-color:#d0ced8;border-left-color:#9b9a96;">'
                '<p style="color:#9b9a96;font-style:italic;">'
                "No significant seismic events (M&thinsp;&ge;&thinsp;6.0) found for the station observation period.</p>"
                "</div>"
            )
...
def station_from_db(
    ...
    show_geodynamics: bool = True,
    show_maps: bool = True,
    maps_out_dir: Optional[str] = None,
) -> StationReport:
    """
    ...
    if media_path and not os.path.isdir(media_path):
        print(
            f" !! Warning: media path does not exist or is not accessible: {media_path}",
            file=sys.stderr,
        )
        print(
            " !! Images (monument, station photos, visit photos) will be omitted.",
            file=sys.stderr,
        )
        media_path = None
    ...
    if meta:
        ...
        monument_path = (
            _media(m.get("monument_image")) if m.get("monument_image") else None
        )
        raw_comments = m.get("comments") or ""
        comments = raw_comments.strip() or None
        navigation_kml_path = (
            _media(m.get("navigation_file")) if m.get("navigation_file") else None
        )
        ...
        for r in instr_rows[:4]:
            ...
            instrument_history.append(
                InstrumentSession(
                    ...
                    ant_h=float(r["AntennaHeight"])
                    if r.get("AntennaHeight") is not None
                    else 0.0,
    ...
    if show_visits:
        ...
        for vr in visit_rows:
            ...
            photo_captions = [
                str(r.get("description") or r.get("name") or "") for r in photos
            ]
...
def _example_station() -> StationReport:
    ...
    return StationReport(
        ...
        logo_path=str(
            Path("/mnt/user-data/uploads/Lowres-GeoDe-Logo-final-13-White.png")
        ),
        ...
        contacts=[
            Contact(
                ...
                "+54 9 2944 809533",
                None,
                "Lives in the estancia (capataz). Main on-site point of contact.",
            ),
            Contact(
                "FB",
                "Francisco Bunge",
            ...
            Contact(
                ...
                "+54 9 2944 808114",
                None,
                "First contact for access requests. Will direct you to Bruno Mele.",
            ),
            Contact(
                "JP",
                "Juan Pablo Parola",
                "Station Manager",
                "+54 9 2995 391666",
                "agrimjuanpabloparola@gmail.com",
                None,
            ),
            Contact(
                "TA",
                ...
                "+54 9 2984 126864",
                None,
                "Not on-site. Contact for internet/connection issues.",
            ),
        ],
        visits=[
            Visit(
                ...
                [
                    "AT47201601090000a.T00",
                    "AT47201601100000a.T00",
                    "AT47201601110000a.T00",
                ],
                ["AT47_2016-01-09.pdf"],
                [
                    asset(f"visit_2016_photo{i}.jpg")
                    for i in range(1, 5)
                    if asset(f"visit_2016_photo{i}.jpg")
                ],
                [
                    "Antenna installation · view south",
                    "Antenna and receiver unit",
                    "Site overview · rocky outcrop",
                    "Antenna detail · view north",
                ],
            ),
            Visit(
                "2015-06-30",
                ...
                [],
                [
                    "AT47201506300000a.T00",
                    "AT47201507010000a.T00",
                    "AT47201507020000a.T00",
                ],
                ["AT47_2015-06-30.pdf"],
                [
                    asset(f"visit_2015_photo{i}.jpg")
                    for i in range(1, 6)
                    if asset(f"visit_2015_photo{i}.jpg")
                ],
                [
                    "Antenna setup · close view",
                    "Site environment · arid terrain",
                    "Antenna with RN40 in background",
                    "Site panorama · view east",
                    "Antenna tripod detail",
                ],
            ),
            Visit(
                "2004-07-27",
                "2004-07-28",
                None,
                ["Diego Denett", "Carlos Gaitan"],
                [],
                ["AT47_2004-07-27.pdf", "AT47_2004-07-28.pdf"],
                [],
                [],
            ),
            Visit(
                ...
                "2002-07-14",
                None,
                ["Horacio Barrera", "Adolfo Garcia"],
                [],
                ["AT47_2002-07-13.pdf", "AT47_2002-07-14.pdf"],
                [],
                [],
            ),
            Visit(
                ...
                "1998-07-18",
                None,
                ["Carlos Gardini", "Knight (first name unconfirmed)"],
                [],
                ["AT47_1998-07-17.pdf", "AT47_1998-07-18.pdf"],
                [],
                [],
            ),
        ],
