...
def yeardoy2fyear(year, doy, hour=12, minute=0, second=0):
    ...
    fractionalYear = (
        year + ((doy - 1) + hour / 24.0 + minute / 1440.0 + second / 86400.0) / diy
    )
...
def fyear2yeardoy(fyear):
    ...
    hh = (days * fractionOfyear - floor(days * fractionOfyear)) * 24.0
    hour = floor(hh)
    mm = (hh - floor(hh)) * 60.0
    minute = floor(mm)
    ss = (mm - floor(mm)) * 60.0
...
def doy2date(year, doy):
    ...
    if doy < 1 or doy > mxd:
        raise pyDateException(
            "day of year input is invalid (year=%i doy=%i)" % (year, doy)
        )
...
class Date(object):
    def __init__(self, **kwargs):
        ...
        for key in kwargs:
            ...
            if key == "year":
                ...
                key == "hour"
            ):  # DDG 03-28-2017: include hour to work with station info object
                self.hour = int(arg)
            elif (
                key == "minute"
            ):  # DDG 03-28-2017: include minute to work with station info object
                self.minute = int(arg)
            elif (
                key == "second"
            ):  # DDG 03-28-2017: include second to work with station info object
                self.second = int(arg)
            elif (
                key == "datetime"
            ):  # DDG 03-28-2017: handle conversion from datetime to pyDate
        ...
        if self.year is not None and self.doy is not None:
            ...
            if self.year == 9999:
                ...
                # compute the fractional year
                self.fyear = yeardoy2fyear(
                    self.year, self.doy, self.hour, self.minute, self.second
                )

                # compute the gps date
                self.gpsWeek, self.gpsWeekDay = date2gpsDate(
            ...
            self.year, self.doy, self.hour, self.minute, self.second = fyear2yeardoy(
                self.fyear
            )
            ...
            raise pyDateException(
                "not enough independent input args to compute full date"
            )
    ...
    def __check_cmp(date) -> "Date":
        if isinstance(date, datetime):
            ...
            raise pyDateException(
                "type: %s invalid. Can only compare pyDate.Date or datetime objects"
                % str(type(date))
            )
    ...
    def __add__(self, ndays):
        if not isinstance(ndays, int):
            raise pyDateException(
                "type: %s invalid.  Can only add integer number of days"
                % str(type(ndays))
            )
    ...
    def __sub__(self, ndays):
        if isinstance(ndays, int):
            ...
            raise pyDateException(
                "type: %s invalid. Can only subtract integer number of days"
                % str(type(ndays))
            )
