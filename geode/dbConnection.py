...
def run_db_migrations(cnn: "Cnn"):
    ...
    if "plate" not in cnn.get_columns("stations").keys():
        ...
        stations = cnn.query_float(
            "SELECT lat, lon, api_id FROM stations", as_dict=True
        )
        for stn in stations:
        ...
        stations = cnn.query_float(
            "SELECT lat, lon, api_id FROM stations "
            "WHERE \"NetworkCode\" NOT LIKE '?%%' AND plate IS NULL",
            as_dict=True,
        )
        for stn in stations:
            if stn["lon"] is not None and stn["lat"] is not None:
    ...
    if fields["hash"].lower() != "bigint":
        ...
        print(
            " >> Converting hash column in ppp_soln to BIGINT. This operation might take a while..."
        )
        cnn.begin_transac()
        cnn.query("""
        ALTER TABLE ppp_soln
    ...
    idx = cnn.query(
        "SELECT * FROM pg_indexes WHERE tablename = 'earthquakes' "
...
class Cnn(object):
    ...
    def update(self, table: str, set_clause_dict: dict, **kwargs):
        ...
        set_clause = ", ".join(
            [f'"{field}" = %s' for field in set_clause_dict.keys() if field in cols]
        )
        ...
        values = list(
            [value for field, value in set_clause_dict.items() if field in cols]
        ) + list(kwargs.values())
