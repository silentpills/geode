...
from ..core.data_classes import AdjustmentResults, LeastSquares
...
from ..least_squares.least_squares import AdjustmentModels, EtmFit
...
def enum_dict_factory(field_list):
    ...
    for key, value in field_list:
        ...
        if isinstance(value, IntEnum) and hasattr(value, "description"):
            result[key] = {"value": value.value, "description": value.description}
        # Handle lists that might contain enums
            ...
            for k, v in value:
                ...
                # Check if value is an IntEnum with a description property
                if isinstance(v, IntEnum) and hasattr(v, "description"):
                    result[key][k] = {"value": v.value, "description": v.description}
        else:
            result[key] = value
    return result
...
class EtmEncoder(json.JSONEncoder):
    ...
    def _round_floats(self, obj, path):
        ...
        if isinstance(obj, float):
            ...
            return {
                key: self._round_floats(value, path=path + [key])
...
class EtmEngine:
    ...
    def __init__(
        self, config: EtmConfig, cnn: Cnn = None, silent: bool = False, **kwargs
    ):
        ...
        polynomial = PolynomialFunction(
            config, time_vector=self.solution_data.time_vector
        )
        ...
        if (
            self.config.modeling.least_squares_strategy.adjustment_model
    ...
    def run_adjustment(
        ...
        if self.config.json_file is None:
            if try_loading_db and cnn and not force_computation:
                ...
                mask = self.config.modeling.get_observation_mask(
                    self.solution_data.time_vector
                )

                success = load_parameters_db(
                    self.config,
                    cnn,
                    self.fit,
        ...
        if not success:
            ...
            if (
                cnn
                and try_save_to_db
                and self.config.modeling.status == FitStatus.POSTFIT
            ):
    ...
    def load_from_json(self, json_: Union[str, dict] = None) -> bool:
        ...
        if "raw_results" in data.keys():
            ...
            for f in [
                ff for ff in self.design_matrix.functions if ff.p.object == "jump"
            ]:
                found = False
    ...
    def save_etm(
        ...
        station_meta.station_information = [
            item.to_json() for item in station_meta.station_information
        ]

        # dump continuous model if requested
        if dump_model:
            model_values = self.fit.get_time_continuous_model(
                self.solution_data.time_vector_cont
            )
        else:
            model_values = None

        functions = [
            asdict(funct.p, dict_factory=enum_dict_factory)
        ...
        etm_dump = {
            ...
            "solution_options": asdict(
                self.config.solution, dict_factory=enum_dict_factory
            ),
            "modeling_params": asdict(
                self.config.modeling, dict_factory=enum_dict_factory
            ),
            "raw_results": [
                asdict(self.fit.results[0]),
                asdict(self.fit.results[1]),
                asdict(self.fit.results[2]),
            ]
            if dump_raw_results
            else None,
        ...
        if filename:
            if not os.path.basename(filename):
                filename = (
                    os.path.join(filename, self.config.build_filename()) + ".json"
                )
            else:
    ...
    def plot(self) -> Union[str, None]:
        ...
        plot_data = PlotDataPreparer(self.config).prepare_time_series_data(
            self.solution_data, self.fit
        )
    ...
    def get_position(
        ...
        if self.config.modeling.status == self.config.modeling.status.POSTFIT:
            for i in range(3):
                model[i] = (
                    self.design_matrix.alternate_time_vector(
                        np.array([date.fyear for date in dates])
                    )
                    @ self.fit.results[i].parameters
                )
        ...
        if etm_solution == EtmSolutionType.OBSERVATION:
            ...
            mask = np.isin(
                self.solution_data.time_vector_mjd, [date.mjd for date in dates]
            )
            i = np.where(mask)[0]
            ...
            if i.size:
                ...
                position = np.array(
                    [
                        self.solution_data.x[i],
                        self.solution_data.y[i],
                        self.solution_data.z[i],
                    ]
                )
                ...
                if model_ecef[0].size:
                    ...
                    sigmas = (
                        np.array(
                            [[result.wrms] * len(dates) for result in self.fit.results]
                        )
                        * limit
                    )
                    ...
                    source = (
                        "No "
                        + stack_name
                        + " solution for requested dates, no ETM: mean coordinate"
                    )
                    position = np.array(
                        [
                            [self.solution_data.x.mean()] * len(dates),
                    ...
                    sigmas = np.array(
                        [
                            [self.solution_data.x.std()] * len(dates),
            ...
            if model_ecef.size:
                ...
                raise Exception(
                    "Model requested by no ETM was found for "
                    + self.config.get_station_id()
                )
    ...
    def query_jump(self, date: Date):
        ...
        for jump in self.design_matrix.functions:
            if (
                jump.p.object == "jump"
                and Date(datetime=jump.p.jump_date) == date
                and jump.fit
            ):
