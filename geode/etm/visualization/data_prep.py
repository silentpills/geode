...
from ..visualization.data_classes import (
    ComponentData,
    PlotOutputConfig,
    TimeSeriesPlotData,
)
...
class PlotDataPreparer:
    ...
    def prepare_time_series_data(
        self, solution_data: SolutionData, etm_fit: EtmFit
    ) -> TimeSeriesPlotData:
        ...
        component_data = self._prepare_all_components(
            local_coords, solution_data, etm_fit
        )
        ...
        if etm_fit:
            plot_data.parameter_summary = self._generate_parameter_summary(
                etm_fit, solution_data
            )
    ...
    def _prepare_all_components(
        self, observations: List, solution_data: SolutionData, etm_results: EtmFit
    ) -> List[ComponentData]:
        """Prepare plot data for all three coordinate components (N, E, U)"""
        mask = etm_results.config.modeling.get_observation_mask(
            solution_data.time_vector
        )
        is_postfit = etm_results.config.modeling.status == FitStatus.POSTFIT

        component_data = []
        for i in range(3):
            data = self._create_base_component_data(
                observations[i], solution_data, mask
            )
    ...
    @staticmethod
    def _create_base_component_data(
        obs: np.ndarray, solution_data: SolutionData, mask: np.ndarray
    ) -> ComponentData:
        ...
        return ComponentData(
            ...
            time_range=(
                solution_data.time_vector.min(),
                solution_data.time_vector.max(),
            ),
            mask=mask,
        )

    @staticmethod
    def _add_model_and_residuals(
        data: ComponentData,
        component_idx: int,
        ...
        model_values = (
            design.alternate_time_vector(solution_data.time_vector_cont)
            @ result.parameters
        ...
        if np.any(~data.mask):
            ...
            model_not_fit = (
                design.alternate_time_vector(solution_data.time_vector[~data.mask])
                @ result.parameters
            ) * M_TO_MM
            data.residuals_not_fit = data.observations_not_fit - model_not_fit

    def _apply_signal_removals(
        self,
        data: ComponentData,
        component_idx: int,
        solution_data: SolutionData,
        ...
        for funct in etm_results.design_matrix.functions:
            ...
            if self._should_remove_signal(obj_type, cfg) and funct.fit:
                self._subtract_signal_from_data(
                    data, component_idx, funct, solution_data.time_vector_cont
                )

            # Handle stochastic signal specially
            if obj_type == "stochastic":
                self._handle_stochastic_signal(
                    data, component_idx, funct, solution_data, etm_results
                )
    ...
    def _should_remove_signal(obj_type: str, cfg: PlotOutputConfig) -> bool:
        ...
        return (
            (obj_type == "periodic" and cfg.plot_remove_periodic)
            or (obj_type == "polynomial" and cfg.plot_remove_polynomial)
            or (obj_type == "jump" and cfg.plot_remove_jumps)
        )

    @staticmethod
    def _subtract_signal_from_data(
        data: ComponentData, component_idx: int, funct, time_vector_cont: np.ndarray
    ) -> None:
        ...
        data.observations_fit -= (
            funct.eval(component_idx, data.time_vector_fit) * M_TO_MM
        )
        if np.any(~data.mask):
            data.observations_not_fit -= (
                funct.eval(component_idx, data.time_vector_not_fit) * M_TO_MM
            )
        data.model_values -= funct.eval(component_idx, time_vector_cont) * M_TO_MM

    def _handle_stochastic_signal(
        self,
        data: ComponentData,
        component_idx: int,
        funct,
        ...
        if self.output_config.plot_remove_stochastic:
            ...
            data.observations_fit -= (
                funct.eval(component_idx, time_mjd[data.mask]) * M_TO_MM
            )
            if np.any(~data.mask):
                data.observations_not_fit -= (
                    funct.eval(component_idx, time_mjd[~data.mask]) * M_TO_MM
            ...
            data.model_values -= (
                etm_results.results[component_idx].stochastic_signal * M_TO_MM
            )
    ...
    def _add_confidence_bounds(
        self, data: ComponentData, component_idx: int, etm_results: EtmFit
    ) -> None:
    ...
    def _generate_parameter_summary(
        etm_results: EtmFit, solution_data: SolutionData
    ) -> str:
