...
class TimeSeriesTemplate(PlotTemplate):
    ...
    def generate_title(
        ...
        if plot_data.has_etm_results and plot_data.parameter_summary:
            ...
            if plot_data.wrms_values:
                wrms_str = (
                    f"NEU wrms [mm]: {plot_data.wrms_values[0]:.2f} "
            ...
            if (
                output_config.plot_remove_stochastic
            ...
            for prefit in self.config.modeling.prefit_models:
                flags.append(
                    self.config.get_label("prefit") + " " + prefit.short_name()
                )

            if len(flags):
                flags_str = (
                    " - " + self.config.get_label("removed") + ": " + ", ".join(flags)
                )
            else:
                flags_str = ""
    ...
    def plot_observations(self, ax, data: ComponentData) -> None:
        ...
        if data.outlier_flags is not None:
            ...
            ax.plot(
                ...
                data.observations_fit[good_mask],
    ...
    def plot_all_observations(self, ax, data: ComponentData) -> None:
        ...
        if data.outlier_flags is not None:
            ...
            if np.any(outlier_mask):
                ax.plot(
                    data.time_vector_fit[outlier_mask],
                    data.observations_fit[outlier_mask],
                    color=self.colors["outliers"],
                    **self.styles["observations"],
                )
        ...
        for jump in jumps:
            if time_range[0] <= jump.date.fyear <= time_range[1]:
                if jump.p.jump_type == JumpType.AUTO_DETECTED:
                    ax.axvline(
                        np.array([jump.date.fyear]),
                        color=jump.p.jump_type.color if jump.fit else "gray",
    ...
    def plot_missing_solutions(self, ax, missing_solutions: List) -> None:
        ...
        for missing_time in missing_solutions:
            ax.axvline(
                missing_time,
                color=self.colors["missing_solution"],
                alpha=0.2,
                linewidth=1,
            )
    ...
    def add_jump_tables(
        ...
        for i, table in enumerate(jump_tables):
            ...
            for j, item in enumerate(reversed(table)):
                ...
                if j < MAX_ITEMS:
                    ...
                    text = fig.text(
                        0.0025,
                        initial_pos,
                        self.config.get_label("table_too_long"),
                        color="black",
                        fontsize=8,
                        family="monospace",
                    )
            ...
            fig.text(
                0.0025,
                initial_pos,
                self.config.get_label("table_title"),
                color="black",
                fontsize=8,
                family="monospace",
            )
