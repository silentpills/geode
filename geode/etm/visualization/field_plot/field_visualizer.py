...
from .data_classes import FieldPlotConfig, PlotState
...
class FieldVisualizer:
    ...
    def plot(
        ...
        self.state.x_stn, self.state.y_stn = self.renderer.transform(
        ...
        if self.config.output_file:
            plt.savefig(
                self.config.output_file, dpi=self.config.dpi, bbox_inches="tight"
            )
    ...
    def _create_figure(self) -> None:
        ...
        self.fig = plt.figure(
            figsize=self.config.figsize, dpi=self.config.dpi, constrained_layout=False
        )
    ...
    def _process_data(self, field_index: int) -> None:
        ...
        valid = ~np.isnan(ve)
        ve_grid = griddata(
            (x[valid], y[valid]), ve[valid], (xi_grid, yi_grid), method="cubic"
        )
        vn_grid = griddata(
            (x[valid], y[valid]), vn[valid], (xi_grid, yi_grid), method="cubic"
        )
        vu_grid = griddata(
            (x[valid], y[valid]), vu[valid], (xi_grid, yi_grid), method="cubic"
        )
        ...
        if self.config.plot_sigmas and self._covar is not None:
            ...
            self.state.cov_grid = griddata(
                (x[valid], y[valid]), cov_en[valid], (xi_grid, yi_grid), method="cubic"
            )
        else:
    ...
    def _draw_subplot(
        self, ax, index: int, label: str, connect_interaction: bool = False
    ) -> None:
        """
    ...
    def _draw_ellipses(self, ax, index: int, scale: float) -> None:
        ...
        for j in range(ve_flat.size):
            ...
            cov_2x2 = np.array(
                [[ve_flat[j] ** 2, cov_flat[j]], [cov_flat[j], vn_flat[j] ** 2]]
            )
    ...
    def _setup_widgets(self) -> None:
        ...
        self.widgets.create_field_selector(
            self._field_names, self.switch_field, initial_index=0
        )

        # Scale slider
        self.widgets.create_scale_slider(self.update_scale, initial_value=50.0)
