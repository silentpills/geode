...
class CartopyRenderer:
    ...
    def setup(
        self, ax, lon: np.ndarray, lat: np.ndarray, pad_fraction: float = 0.1
    ) -> None:
        """
    ...
    def add_features(self, ax) -> None:
        ...
        # Add land with fill
        ax.add_feature(
            cfeature.LAND.with_scale(self.resolution), facecolor="wheat", alpha=0.3
        )

        # Add ocean
        ax.add_feature(
            cfeature.OCEAN.with_scale(self.resolution), facecolor="lightblue", alpha=0.3
        )

        # Add coastlines
        ax.add_feature(cfeature.COASTLINE.with_scale(self.resolution), linewidth=0.5)

        # Add country borders
        ax.add_feature(cfeature.BORDERS.with_scale(self.resolution), linewidth=0.5)
        ...
        ax.add_feature(
            states_provinces, edgecolor="dimgray", linewidth=0.4, linestyle=":"
        )
    ...
    def transform_grid(
        self, lon_grid: np.ndarray, lat_grid: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    @staticmethod
    def mask_ocean_points(
        lon: np.ndarray, lat: np.ndarray, buffer_distance: float = 0.0
    ) -> np.ndarray:
        """
        ...
        if buffer_distance > 0:
            ...
            world_proj["geometry"] = world_proj.geometry.buffer(
