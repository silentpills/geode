...
class HistogramTemplate(PlotTemplate):
    ...
    def plot_ne_scatter(
        self, fig, ax, plot_data: TimeSeriesPlotData
        ...
        mask = (
            combined_residuals
            <= np.sqrt(np.sum(np.square(plot_data.wrms_values))) * 3.5
        )  # 50mm

        ax.plot(
            e_res[mask], n_res[mask], "o", color=(0, 150 / 255, 235 / 255), markersize=2
        )
        ...
        ax.set_title(
            f"{self.config.get_label('residual plot')} "
    ...
    def plot_component_histograms(
        ...
        components = [
            (
                "n_hist",
                plot_data.north_data.residuals,
                self.config.get_label("north"),
                plot_data.north_data.outlier_flags,
            ),
            (
                "e_hist",
                plot_data.east_data.residuals,
                self.config.get_label("east"),
                plot_data.east_data.outlier_flags,
            ),
            (
                "u_hist",
                plot_data.up_data.residuals,
                self.config.get_label("up"),
        ...
        for ax_key, residuals, label, outliers in components:
            ...
            ax.hist(
                ...
                orientation="horizontal" if "n_hist" in ax_key else "vertical",
            )
            ...
            if "n_hist" in ax_key:
                ax.plot(
                    p, x, "r", linewidth=2, label=f"Fit: mu={mu:.2f}, std={std:.2f}"
                )
            else:
                ax.plot(
                    x, p, "r", linewidth=2, label=f"Fit: mu={mu:.2f}, std={std:.2f}"
                )
    ...
    def _plot_error_ellipse(
        ax, covariance: np.ndarray, center_e: float, center_n: float
    ) -> None:
        ...
        cov_en = np.array(
            [[cov_en[1, 1], cov_en[1, 0]], [cov_en[0, 1], cov_en[0, 0]]]
        )  # Swap for E-N order
