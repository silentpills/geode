...
class EtmPlotter:
    ...
    def plot_time_series(
        ...
        output_config: Optional[PlotOutputConfig] = None,
    ) -> Optional[str]:
        """
        ...
        if plot_data.jump_tables:
            template.add_jump_tables(
                fig, main_axes, plot_data.jump_tables, plot_data.jumps
            )
    ...
    def _plot_component_data(
        self,
        ax,
        plot_data: TimeSeriesPlotData,
        component: str,
        component_idx: int,
        output_config: PlotOutputConfig,
    ) -> None:
        ...
        if output_config.plot_residuals_mode:
            ...
            if (
                ...
                and not self.config.plotting_config.plot_no_model
            ):
    ...
    def _handle_plot_output(
        self, fig, output_config: Optional[PlotOutputConfig], histogram: bool = False
    ) -> Optional[str]:
        ...
        if output_config.filename:
            if not os.path.basename(output_config.filename):
                filename = (
                    os.path.join(output_config.filename, self.config.build_filename())
                    + ("_hist" if histogram else "")
                    + "."
