...
def save_parameters_db(cnn: Cnn, etm_results: EtmFit) -> None:
    ...
    var_factor = cnn.query_float(
        query.format(
            etm_results.config.network_code,
            etm_results.config.station_code,
            etm_results.config.solution.solution_type.code,
            "var_factor",
            etm_results.config.solution.stack_name,
        ),
        as_dict=True,
    )
    if var_factor:
...
def load_parameters_db(
    ...
    for funct in dm.functions:
        obj = next(
            (
                item
                for item in etms
                if item.get("object") == funct.p.object
                and item.get("jump_date") == funct.p.jump_date
                and item.get("jump_type") == funct.p.jump_type
            ),
            None,
        )
        ...
        if obj and obj["params"]:
            ...
            for i in range(3):
                ...
                if funct.p.object == "stochastic":
                    etm_results.results[i].stochastic_signal = np.array(
                        obj["params"][i]
                    ).astype(float)
    ...
    for i in range(3):
        etm_results.results[i].origin = (
            cnn.options["hostname"] + ":" + cnn.options["database"]
        )
        etm_results.results[i].parameters = np.array(var_factor["params"][i]).astype(
            float
        )
        ...
        v = observations[i] - dm.matrix @ np.array(var_factor["params"][i]).astype(
            float
        )
        ...
        etm_results.results[i].obs_sigmas = (
            np.ones(observations[i].shape[0]) * etm_results.results[i].wrms
        )
    ...
    etm_results.outlier_flags = np.all(
        (
            etm_results.results[0].outlier_flags,
            etm_results.results[1].outlier_flags,
            etm_results.results[2].outlier_flags,
        ),
        axis=0,
    )
