...
from ..core.type_declarations import (
    AdjustmentModels,
    CovarianceFunction,
    FitStatus,
    JumpType,
...
class AdjustmentStrategy(ABC):
    ...
    def create_instance(cls, config: EtmConfig) -> "AdjustmentStrategy":
        ...
        if strategy == AdjustmentModels.ROBUST_LEAST_SQUARES:
            ...
            raise ValueError(
                f"Adjustment strategy type {strategy.description} not implemented"
            )
    ...
    def compute_plomb(residuals, time_vector_mjd) -> float:
        ...
        T_total = np.max(time_vector_mjd) - np.min(
            time_vector_mjd
...
class EmpiricalCovariance:
    def __init__(
        ...
        self.bins_mjd, self.bins_fyr, self.empirical_covariance = (
            self._compute_empirical_covariance(
                time_vector_mjd[outlier_flags], residuals[outlier_flags]
            )
        )
    ...
    def get_covariance_matrix(self, time_vector_mjd: np.ndarray):
        if self.method == CovarianceFunction.ARMA and not self.arma_failed:
            ...
            while roots > 3:
                try:
                    ...
                    csz, empirical_cov_function = self.arma_compute(
                        pk, Ak, time_vector_mjd
                    )

                    np.linalg.cholesky(csz)
                    logger.debug(
                        f"EmpiricalCovariance: Matrix is symmetric positive definite with {roots} roots "
                        f"and {self.n} points"
                    )
                    ...
                    logger.debug(
                        f"EmpiricalCovariance: Bad roots found for {roots} roots and {self.n} points, "
                    ...
                    logger.debug(
                        f"EmpiricalCovariance: Matrix is not symmetric positive definite for {roots} roots "
            ...
            logger.debug(
                "EmpiricalCovariance: Could not find symmetric positive definite matrix using ARMA, "
        ...
        if (
            self.method in (CovarianceFunction.GAUSSIAN, CovarianceFunction.CAUCHY)
            or self.arma_failed
        ):
...
class RobustLeastSquares(AdjustmentStrategy):
    ...
    def adjust(
        ...
        **kwargs,
        ...
        if self.dof == 0:
            raise AdjustmentStrategyException(
                "Degrees of freedom == 0. Cannot fit model."
            )
        ...
        for j in range(iterations):
            ...
            results.parameters = np.linalg.solve(
                aw.T @ aw + n_neq_left, aw.T @ lw + c_neq_right
            )
            ...
            logger.debug(
                f"RobustLeastSquares sqrt(so): {self.so:.4f} it: {j:2d} pass: {self.x1 <= x <= self.x2}"
            )
        ...
        try:
            if not results.converged:
                logger.info(
                    f"RobustLeastSquares did not converge! final wrms {wrms / 1000:.1f} mm"
                )

            results.covariance_matrix = np.linalg.inv(a.T @ weights.matrix @ a) * (
                self.so**2
            )
        except np.linalg.LinAlgError as e:
            logger.info("np.linalg.inv failed to obtain covariance matrix: %s" % str(e))
            results.covariance_matrix = np.ones(
                (design_matrix.matrix.shape[1], design_matrix.matrix.shape[1])
            )
        ...
        logger.debug(
            f"{' ':<28}: "
            + " ".join(
                [funct.short_name() for funct in design_matrix.functions if funct.fit]
            )
        )
        logger.debug(
            f"{'RobustLeastSquares [mm]':>28}: "
            + " ".join([f"{p * 1000.0:10.3e}" for p in results.parameters.tolist()])
        )

        logger.info(
            f"RobustLeastSquares rejected outliers: "
...
class LeastSquaresCollocation(AdjustmentStrategy):
    def adjust(
        ...
        **kwargs,
        ...
        logger.debug(
            f"LeastSquaresCollocation least squares in {time() - tt:.3f} s "
            f"sigma_o: {self.so:.3f} "
            f"wrms: {results.wrms * 1000.0} mm"
        )
        ...
        logger.info(
            f"LeastSquaresCollocation rejected outliers: "
            f"{np.sum(~results.outlier_flags) / results.outlier_flags.shape[0] * 100:.1f}%"
        )
    ...
    def _least_squares(
        ...
        idx = int(
            a.shape[0] / 1
        )  # np.where(cov[0, :] <= (np.finfo(float).eps / 1000))[0].min()
        ...
        for i, e in enumerate(np.cumsum(chunks).tolist()):
            ...
            tc = time_vector_cont_mjd[
                np.logical_and(
                    time_vector_cont_mjd >= tv.min(), time_vector_cont_mjd <= tv.max()
                )
            ]
        ...
        results.covariance_matrix = np.linalg.solve(
            n, np.eye(results.parameters.shape[0])
        ) * (self.so**2)
...
class EtmFit:
    def __init__(
        ...
        config: EtmConfig,
        design_matrix: DesignMatrix,
        solution_data_hash: int = None,
    ):
        self.config = config
        self.outlier_flags: np.ndarray = np.array([])
        self.results: List[AdjustmentResults] = [
            AdjustmentResults(),
            AdjustmentResults(),
            AdjustmentResults(),
        ]
        ...
        try:
            while True:
                ...
                for f, message in validation:
                    logger.info(message)
                    if message.startswith("Unrealistic amplitude") and isinstance(
                        f, JumpFunction
                    ):
                        if not f.was_modified:
                            f.configure_behavior(
            ...
            logger.info(
                self.config.modeling.status.description
                + " "
    ...
    def load_results_to_functions(self):
        ...
        # single outlier flag
        self.outlier_flags = np.all(
            (
                self.results[0].outlier_flags,
                self.results[1].outlier_flags,
                self.results[2].outlier_flags,
            ),
            axis=0,
        )
    ...
    def _validate_function_design_matrix(self):
        ...
        for i in range(3):
            ...
            for f, message in validation:
                logger.info(message)
                if message.startswith("Condition number too large") and isinstance(
                    f, JumpFunction
                ):
                    ...
                    if not self.config.modeling.check_jump_collisions:
                        ...
                        logger.info(
                            "Adding internal constraint = 0 for stabilizing system"
                        )
                        ...
                        logger.info(
                            "Function has a single relaxation, cannot remove it."
                        )
            if not check_again:
                break
        ...
        if check_again:
            logger.info(
                "While checking the etm functions design matrices the max iteration "
    ...
    def get_var_factor_db_fields(self):
        ...
        return {
            ...
            "sigmas": [
                [float(result.variance_factor) for result in self.results],
                [float(result.wrms) for result in self.results],
            ],
            "hash": self.hash,
        }

    def get_time_continuous_model(
        self, time_vector_cont: np.ndarray
    ) -> List[np.ndarray]:
        # spit the auto position by default, in case there is no model
        model = [np.array([0]), np.array([0]), np.array([0])]

        if self.config.modeling.status == FitStatus.POSTFIT:
            for i in range(3):
                model[i] = (
                    self.design_matrix.alternate_time_vector(time_vector_cont)
                    @ self.results[i].parameters
                )
    ...
    def _detect_outliers(
        self, observations: np.ndarray, results: "AdjustmentResults"
    ) -> None:
    ...
    def process_covariance(self):
        ...
        for k, i, j in ((0, 0, 1), (1, 1, 2), (2, 0, 2)):
            cov[k] = (
                np.corrcoef(self.results[i].residuals[f], self.results[j].residuals[f])[
                    0, 1
                ]
                * self.results[i].wrms
                * self.results[j].wrms
