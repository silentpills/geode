...
from .data_classes import EtmStackerConfig, EtmStackerField, NormalEquations, Station
...
class EtmStacker:
    ...
    def __init__(self, config: EtmStackerConfig = None):
        ...
        self.collided_earthquakes: Set[str] = (
            set()
        )  # IDs of earthquakes that lost collision check
    ...
    def _build_etm(
        ...
        if json_path is not None and os.path.isfile(json_path):
            ...
            try:
                ...
                unconstrained_config.modeling.relaxation = np.array(
                    [np.max(self.config.relaxation)]
                )
                etm_unconstrained = EtmEngine(
                    unconstrained_config, cnn=cnn, silent=True
                )

                if (
                    etm_unconstrained.solution_data.time_vector[-1]
                    - etm_unconstrained.solution_data.time_vector[0]
                    <= 1.5
                ):
                    tqdm.write(
                        print_yellow(
                ...
                etm_unconstrained.run_adjustment(
            ...
            if etm_unconstrained.config.modeling.status == FitStatus.UNABLE_TO_FIT:
                tqdm.write(
                    print_yellow(
                        f" -- Unconstrained ETM for {network_code}.{station_code} "
                ...
                saved_obs_sigmas = [
                    r.obs_sigmas.copy() for r in etm_unconstrained.fit.results
                ]
                prefit = list(
                    etm_unconstrained.jump_manager.get_active_mechanical_jumps()
                )
                if prefit:
                    tqdm.write(
                        f" -- Found {len(prefit)} mechanical jump(s) in {network_code}.{station_code}; "
        ...
        try:
            ...
            etm.run_adjustment(
                cnn=cnn,
                force_computation=not loaded_from_json,
                try_loading_db=False,
                try_save_to_db=False,
            )
            etm.config.plotting_config.filename = (
                f"./production/{network_code}.{station_code}_stacker"
            )
            etm.plot()
        except (DesignMatrixException, np.linalg.LinAlgError):
            tqdm.write(
                print_yellow(
        ...
        if etm.config.modeling.status == etm.config.modeling.status.UNABLE_TO_FIT:
            tqdm.write(
                print_yellow(
                    f" -- Unable to fit station {network_code}.{station_code}. "
            ...
            if not zero_constraints:
                tqdm.write(
                    print_yellow(
                        f" -- No geophysical jumps to constrain for "
            ...
            try:
                ...
                tqdm.write(
                    print_yellow(
                        f" -- Still unable to fit {network_code}.{station_code} with constraints. "
                        f"Station will not be added."
                    )
                )
                return None
            if etm.config.modeling.status == etm.config.modeling.status.UNABLE_TO_FIT:
                tqdm.write(
                    print_yellow(
                        f" -- Still unable to fit {network_code}.{station_code} after constraints. "
                        f"Station will not be added."
                    )
                )
                return None
    ...
    def _apply_config(
        self, network_code: str, station_code: str, cnn: Cnn
    ) -> EtmConfig:
    ...
    def prepare_earthquake_list(self, cnn: Cnn, stnlist: list) -> None:
        ...
        tqdm.write(
            f"Pre-computing earthquake list for {len(stnlist)} stations "
        ...
        for stn in stnlist:
            ...
            if not rs:
                tqdm.write(
                    f"  WARNING: Could not get coordinates for {net}.{code}, skipping"
                )
                continue
        ...
        tqdm.write(
            f"  s-score scan: {len(candidate_events)} unique events from "
        ...
        if self.config.earthquakes_cherry_picked:
            for eid in self.config.earthquakes_cherry_picked:
                ...
                if not rows:
                    tqdm.write(
                        f"  WARNING: Cherry-picked event {eid} not found in database"
                    )
                    continue
                j = rows[0]
        ...
        if no_fm:
            tqdm.write(
                f"  Excluding {len(no_fm)} event(s) with no focal mechanism: "
        ...
        for i, event_i in enumerate(candidate_events):
            for j, event_j in enumerate(candidate_events):
                ...
                if days_apart <= collision_window_days:
                    ...
                    shared = event_stations.get(event_i.id, set()) & event_stations.get(
                        event_j.id, set()
                    )
                    if not shared:
                        tqdm.write(
        ...
        if self.collided_earthquakes:
            tqdm.write(
                f"  Collisions: {len(self.collided_earthquakes)} event(s) excluded: "
        ...
        tqdm.write(
            f"Earthquake list ready: {len(kept)} events "
    ...
    def _register_constraints(self):
        ...
        for event in self.earthquakes:
            ...
            if event.id in self.collided_earthquakes:
                tqdm.write(
                    f"Setting number of stations for collided earthquake {event.id} to zero."
                )
            ...
            fault_geometry = FaultGeometry(
                event, self.stations, np.max(self.config.relaxation), self.grids
            )
            ...
            for relax in self.config.relaxation:
                ...
                if not stations:
                    tqdm.write(
                        f"No stations for postseismic event {event.id} relax={relax:.3f}. "
    ...
    def update_weights(
        ...
        for constraint in apply_to:
            tqdm.write(
                f"Updating sigma for {constraint.short_description()}: "
    ...
    def update_okada_weights(
        ...
        for constraint in apply_to:
            tqdm.write(
                f"Updating Okada weights for {constraint.short_description()}: "
            ...
            for constraint_type in constraint_types:
                for constraint in constraint_type:
                    if constraint.is_collision:
                        tqdm.write(
                            f"Skipping field interpolation for collided event {constraint.event.id} "
                            f"(parameters are zero-tied)"
                        )
                        continue

                    if (
                        constraint.constraint_type != ConstraintType.INTERSEISMIC
                        and getattr(constraint, "dislocation_model", None) is None
                    ):
                        tqdm.write(
                            f"Skipping field interpolation for {constraint.short_description()} "
