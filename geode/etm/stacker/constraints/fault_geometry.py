...
from ....Utils import azimuthal_equidistant, stationID
...
class FaultGeometry:
    ...
    def __init__(
        ...
        self.down_dip = (
            10.0 ** (-1.01 + 0.32 * event.magnitude) * 1.6
        )  # [km] (inflate 60%)
        ...
        self.station_list = [
            stn for stn in stations if stn.get_coseismic_column(event.id) is not None
        ]
        ...
        if len(self.station_list) < 2:
            self.station_list = [
                stn
                for stn in stations
                if stn.get_postseismic_column(event.id, relaxation) is not None
            ]
        ...
        if len(self.station_list) < 2:
            ...
            msg = "No stations have" if not self.station_list else "Only 1 station has"
            tqdm.write(
                f"  WARNING: {msg} a jump for {self.event.id}; "
                f"cannot determine plane — defaulting to plane 0"
            )
            self.plane = 0
            self._compute_patch_grids(
                len(self.station_list) if self.station_list else 1
            )
    ...
    def _compute_patch_grids(self, n_stations: int):
        ...
        tqdm.write(
            f"Event {self.event.id} {self.event.date.yyyyddd()}: {n_stations} stations -> "
            f"{n_patches_h} horizontal patches, {n_patches_v} vertical patches"
        )
        tqdm.write(
            f"  Fault dimensions: AS={self.along_strike:.1f} km, DD={self.down_dip:.1f} km"
        )
    ...
    def _build_patch_grid(self, n_patches: int, verbose: bool = True) -> PatchGrid:
        ...
        if verbose:
            tqdm.write(
                f"    Rectangular patch grid: {ps}×{ps} = {actual_patches} patches "
        ...
        for dip in self.event.dip:
            ...
            if np.any(fault_dep.round() <= 0):
                ...
                adjustment = min_dep - radius_W * sind(dip) - 1.0
    ...
    def determine_plane(
        ...
        for idx, j in enumerate(jumps):
            ...
            if (
                s_n < sigma_threshold
                and s_e < sigma_threshold
                and s_u < sigma_threshold
            ):
                keep_idx.append(idx)
            else:
        ...
        if filtered_sites:
            tqdm.write(
                f"  Stations excluded from plane determination "
        ...
        if len(keep_idx) < 2:
            ...
            if keep_idx:
                tqdm.write(
                    f"  WARNING: Only {len(keep_idx)} station passes the sigma filter "
                    f"(minimum 2 required); using all stations for plane determination"
                )
            else:
                tqdm.write(
                    "  WARNING: No stations pass the sigma filter; "
        ...
        tqdm.write(
            f"Testing fault planes for {self.event.id} with {N} stations "
        ...
        for i in range(N):
            obs_s = " ".join(
                [f"{obs_mat[j, i] * 1000.0:>{col_w}.1f}" for j in range(3)]
            )
            plane_parts = []
            for k in range(2):
                ...
                mod_s = " ".join(
                    [f"{m_mat[j, i] * 1000.0:>{col_w}.1f}" for j in range(3)]
                )
                res_s = " ".join(
                    [f"{v_mat[j, i] * 1000.0:>{col_w}.1f}" for j in range(3)]
                )
                plane_parts.append(f"{mod_s}   {res_s}")
            tqdm.write(
                f"  {sites_nam[i]:<12} | {obs_s} | {plane_parts[0]} | {plane_parts[1]}"
            )
        ...
        tqdm.write(
            f"Horizontal RMS: plane0={rms_list[0] * 1000:.2f}mm, plane1={rms_list[1] * 1000:.2f}mm"
        )
        tqdm.write(
            f"Selected plane {self.plane}: strike={self.event.strike[self.plane]:.1f}, "
            f"dip={self.event.dip[self.plane]:.1f}"
        )

        return a_list[self.plane], p_list[self.plane]

    def _compute_sw_okada_system(
        self,
        grid: "GridSystem",
        stnlon: np.ndarray,
        stnlat: np.ndarray,
        ...
        local_patches_h = self._build_patch_grid(
            max(1, int(np.floor(N / 2))), verbose=False
        )
        local_patches_v = self._build_patch_grid(
            max(1, int(np.floor(N / 4))), verbose=False
        )

        # Compute Okada response matrices using local patch grids
        D_ok = self._compute_okada_horizontal(
            tx, ty, dip, grid.poisson_ratio, patches=local_patches_h
        )
        D_oz = self._compute_okada_vertical(
            tx, ty, dip, grid.poisson_ratio, patches=local_patches_v
        )
        ...
        v_weight_eff = (
            (5.0 if v_weight is None else v_weight) * term_av / max(term_Poz, 1e-10)
        )
    ...
    def _compute_okada_horizontal(
        self,
        tx: np.ndarray,
        ty: np.ndarray,
        dip: float,
        poisson_ratio: float,
        patches: "PatchGrid" = None,
    ) -> np.ndarray:
        """
    ...
    def _rotate_okada_horizontal(
        self, D_ok: np.ndarray, R: np.ndarray, N: int
    ) -> np.ndarray:
        """
