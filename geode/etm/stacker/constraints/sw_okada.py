...
class SWOkada(BaseConstraint):
    ...
    def get_parameters_and_covariance(
        self, solution: np.ndarray, covariance: np.ndarray
    ):
    ...
    def _compute_dislocation_model(self, grids: "GridSystem", mask: np.ndarray):
        ...
        self.dislocation_model = self.fault_geometry._compute_sw_okada_system(
            ...
            mask,
            self.spline_tension,
            self.sw_okada_h_weight,
            self.sw_okada_v_weight,
        )
    ...
    def _compute_grid_prediction_kernels(
        self, stations: List[Station], mask: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        x, y = azimuthal_equidistant(
            np.array(self.event.lon), np.array(self.event.lat), sites_lon, sites_lat
        )
        ...
        a_dagger_h = a_dagger[
            : 2 * N, : 2 * N
        ]  # maps [E_obs, N_obs] -> horizontal coefficients
        a_dagger_v = a_dagger[
            2 * N :, 2 * N :
        ]  # maps  U_obs         -> vertical coefficients
        ...
        r_grid_to_stn = np.abs(
            (grid_x_epi[:, None] - x) + 1j * (grid_y_epi[:, None] - y)
        )
        ...
        if np.any(np.isnan(ke)) or np.any(np.isnan(kn)) or np.any(np.isnan(ku)):
            tqdm.write(
                f"WARNING: NaN detected in grid prediction kernels for event {self.event.id}"
            )

        return ke, kn, ku

    def _compute_interpolation_coefficients(
        self,
        target_station: Station,
        constraining_stations: List[Station],
        mask: np.ndarray,
        ...
        other_stations = [
            stn for i, stn in enumerate(constraining_stations) if i != idx
        ]
        ...
        x_other, y_other = azimuthal_equidistant(
            np.array(self.event.lon), np.array(self.event.lat), other_lon, other_lat
        )
        x_target, y_target = azimuthal_equidistant(
            np.array(self.event.lon), np.array(self.event.lat), target_lon, target_lat
        )

        # Local offset from the N-1 other stations.  Guard for N_other < 2.
        if N_other >= 2:
            r_ev, _, _ = get_radius(
                np.column_stack([x_other, y_other]), np.column_stack([x_other, y_other])
            )
            np.fill_diagonal(r_ev, np.inf)
            local_reg = max(8.0, float(np.median(r_ev.min(axis=1))) * 0.5)
        else:
        ...
        length_scale = (
            np.abs(
                np.max(grid_x_masked)
                - np.min(grid_x_masked)
