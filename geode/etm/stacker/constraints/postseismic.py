    ...
    def select_stations(
        ...
        for stn in all_stations:
            ...
            if (
                ...
                has_early_data = (
                    np.min(post_event - jump.date.mjd) <= MISSING_DAYS_TOLERANCE
                )

                if has_early_data:
                    post_event_span_years = (
                        np.max(post_event) - jump.date.mjd
                    ) / 365.25
                    if (
                        event_is_recent
                        or post_event_span_years >= MIN_CONSTRAINING_YEARS
                    ):
                        constraining.append(stn)
                    else:
                        tqdm.write(
                            f" -- {stn.network_code}.{stn.station_code}: moved to to_constrain "
        ...
        tqdm.write(
            f"PostseismicConstraint({self.event.id} {self.relaxation:.3f}): "
    ...
    def get_parameters_and_covariance(
        ...
        for stn in self._station_list:
            par, _ = stn.get_constrained_relax(
                self.event, self.relaxation, solution, covariance
            )
            if par is not None:
