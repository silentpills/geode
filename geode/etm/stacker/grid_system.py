...
from ...Utils import azimuthal_equidistant, inverse_azimuthal, stationID
...
class GridSystem:
    ...
    def create_from_stations(
        ...
        tqdm.write(
            f"Station spacing statistics -> mean NN: {np.mean(nn_dist):.1f} km, "
    ...
    @staticmethod
    def compute_spline_greens(
        stations_projected: np.ndarray, tension=0.10
    ) -> np.ndarray:
        ...
        length_scale = (
            np.abs(
                np.max(x.flatten())
                - np.min(x.flatten())
                + 1j * (np.max(y.flatten()) - np.min(y.flatten()))
            )
            / 50
        )
    ...
    def compute_sw_design_matrix(
        self, source_x: np.ndarray, source_y: np.ndarray, offset: float = None
    ) -> np.ndarray:
    ...
    def predict_seismic_deformation(
        self,
        event: Earthquake,
        stations: List["Station"],
        observations: np.ndarray,
        covar: np.ndarray,
        constraint: Union["CoseismicConstraint", "PostseismicConstraint"],
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Output enu_cov is en, eu, nu."""
        tqdm.write(
            f"Predicting from {constraint.constraint_type} for event {event.id} {event.date.yyyyddd()}"
        )

        sites = np.isin(
            [stationID(s) for s in constraint.station_list],
        ...
        a_e = np.hstack(
            [ke, np.zeros((active_points, N_obs))]
        )  # ke maps [L_e, L_n], pad for L_u
        a_n = np.hstack(
            [kn, np.zeros((active_points, N_obs))]
        )  # kn maps [L_e, L_n], pad for L_u
        a_u = np.hstack(
            [np.zeros((active_points, 2 * N_obs)), ku]
        )  # ku maps L_u only, pad for L_e, L_n
