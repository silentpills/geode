...
from ..core.data_classes import AdjustmentResults, BaseDataClass, Earthquake
...
class EtmStackerConfig(BaseDataClass):
    ...
    post_seismic_back_lim: Union[float, Date] = field(
        default_factory=lambda: Date(year=1990, doy=1)
    )
    ...
    postseismic_h_sigma: float = (
        0.001  # 1 mm — matches PostseismicConstraint class default
    )
    postseismic_v_sigma: float = (
        0.003  # 3 mm — matches PostseismicConstraint class default
    )
...
class EtmStackerField(BaseDataClass):
    ...
    def create_field(
        ...
        grids: "GridSystem",
        constraint: Union[
            "InterseismicConstraint", "CoseismicConstraint", "PostseismicConstraint"
        ],
    ):
        ...
        if constraint.constraint_type == ConstraintType.INTERSEISMIC:
            ...
            return EtmStackerField(
                ...
                constrained_parameters=params,
                enu_covar=velocity_cova,
            )
        else:
            seismic_field, seismic_sigma, seismic_covar = (
                grids.predict_seismic_deformation(
                    constraint.event,
                    constraint.station_list,
                    params,
                    sigmas,
                    constraint,
                )
            ...
            return EtmStackerField(
                ...
                grids,
                enu_field=seismic_field,
                onset_date=constraint.event.date,
                enu_sigma=seismic_sigma,
                constrain_stations=constraint.station_list,
                event=constraint.event,
                relaxation=constraint.relaxation,
                constrained_parameters=params,
                enu_covar=seismic_covar,
            )
