...
# app
from . import pyEvents, pyProducts, pyRinex, pyRunWithRetry
from .pyDate import Date
from .Utils import determine_frame, ecef2lla, file_readlines, file_write, lg2ct
...
class PPPSpatialCheck:
    ...
    def verify_spatial_coherence(self, cnn, StationCode, search_in_new=False):
        ...
        )  # DO NOT RETURN RESULTS NetworkCode = '?%'
...
class RunPPP(PPPSpatialCheck):
    def __init__(
        ...
        apply_met=True,
        kinematic=False,
        clock_interpolation=False,
        hash=0,
        erase=True,
        decimate=True,
        solve_coordinates=True,
        ...
        self.rootdir = os.path.join(
            os.path.join("production", "ppp"), str(uuid.uuid4())
        )
        ...
        self.solve_troposphere = (
            1 if observations == OBSERV_CODE_ONLY else solve_troposphere
        )
        self.back_substitution = (
            False if observations == OBSERV_CODE_ONLY else back_substitution
        )
        ...
        if in_rinex.rinex_version >= 3:
            ...
            rinexobj = pyRinex.ReadRinex(
                in_rinex.NetworkCode,
                in_rinex.StationCode,
                in_rinex.origin_file,
                no_cleanup=in_rinex.no_cleanup,
                allow_multiday=in_rinex.allow_multiday,
            )
        ...
        rinexobj.window_data(
            rinexobj.date.first_epoch("datetime"), rinexobj.date.last_epoch("datetime")
        )
        ...
        if observations == OBSERV_CODE_PHASE:
            self.clock_interpolation = (
                clock_interpolation
                if rinexobj.date > Date(year=2001, month=5, day=1)
                else False
            )
        else:
        ...
        if os.path.isfile(self.rinex.rinex_path):
            ...
            try:
                ...
                if sp3altrn:
                    ...
                    raise type(e)(
                        str(e)
                        + " -> This exception usually occurs due to the need of having "
            ...
            copyfile(
                self.rinex.rinex_path, os.path.join(self.rootdir, self.rinex.rinex)
            )

        else:
            raise pyRunPPPException(
                "The file "
                + self.rinex.rinex_path
    ...
    def copyfiles(self):
        ...
        for f in files:
            if os.path.exists(os.path.join(self.ppp_path, f)):
                copyfile(os.path.join(self.ppp_path, f), os.path.join(self.rootdir, f))
            else:
        ...
        copyfile(
            os.path.join(self.atx),
            os.path.join(self.rootdir, os.path.basename(self.atx)),
        )

    def write_otl(self):
        file_write(
            os.path.join(self.rootdir, self.rinex.StationCode + ".olc"), self.otl_coeff
        )
    ...
    def config_session(self):
        ...
        # create the def file
        file_write(
            os.path.join(self.rootdir, "gpsppp.def"),
            "'LNG' 'ENGLISH'\n"
            "'TRF' 'gpsppp.trf'\n"
        ...
        file_write(
            os.path.join(self.rootdir, "commands.cmd"),
        ...
        file_write(
            os.path.join(self.rootdir, "input.inp"),
            "%s\n"
            "commands.cmd\n"
            "0 0\n"
            "0 0\n"
            ...
            % (
                self.rinex.rinex,
                self.orbits1.filename,
                self.clocks1.filename,
                self.orbits2.filename,
                self.clocks2.filename,
            ),
        )
    ...
    def get_orbits(self, orbit_type):
        ...
        if self.observations == OBSERV_CODE_PHASE:
            orbits1 = pyProducts.GetSp3Orbits(
                options["sp3"], self.rinex.date, orbit_type, orbits_path, True
            )
            orbits2 = pyProducts.GetSp3Orbits(
                options["sp3"], self.rinex.date + 1, orbit_type, orbits_path, True
            )

            clocks1 = pyProducts.GetClkFile(
                options["sp3"], self.rinex.date, orbit_type, orbits_path, True
            )
            clocks2 = pyProducts.GetClkFile(
                options["sp3"], self.rinex.date + 1, orbit_type, orbits_path, True
            )
        else:
            # for code-only solution we get the BRDC orbit and we use the same information for all files.
            orbits1 = pyProducts.GetBrdcOrbits(
                options["brdc"], self.rinex.date, orbits_path, True
            )
            orbits2 = orbits1
            clocks1 = orbits1
            clocks2 = orbits1
        try:
            eop_file = pyProducts.GetEOP(
                options["sp3"], self.rinex.date, orbit_type, self.rootdir
            )
    ...
    def get_xyz(section):
        x = re.findall(
            r"X\s\(m\)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)",
            section,
        )[0][1]
        y = re.findall(
            r"Y\s\(m\)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)",
            section,
        )[0][1]
        z = re.findall(
            r"Z\s\(m\)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)",
            section,
        )[0][1]
    ...
    def get_clock(section, kinematic):
        ...
        try:
            clock_phase = re.findall(
                r"Clock Phase\s*\([nm]s\)\s*:\s*(-?\d+\.\d+)\s*(-?\d+\.\d+)", section
            )[0]
            phase_drift = re.findall(
                r"Phase Drift\s*\([nm]s/day\)\s*:\s*(-?\d+\.\d+)\s*(-?\d+\.\d+)",
                section,
            )[0]
            clock_resid = re.findall(
                r"RMS residuals\s*\([nm]s\)\s*:\s*(-?\d+\.\d+)\s*(\d+)", section
            )[0]
    ...
    def get_sigmas(section, kinematic):
        if kinematic:
            sx = re.findall(
                r"X\s\(m\)\s+-?\d+\.\d+\s+-?\d+\.\d+\s+(-?\d+\.\d+|[nN]a[nN]|\*+)",
                section,
            )[0]
            sy = re.findall(
                r"Y\s\(m\)\s+-?\d+\.\d+\s+-?\d+\.\d+\s+(-?\d+\.\d+|[nN]a[nN]|\*+)",
                section,
            )[0]
            sz = re.findall(
                r"Z\s\(m\)\s+-?\d+\.\d+\s+-?\d+\.\d+\s+(-?\d+\.\d+|[nN]a[nN]|\*+)",
                section,
            )[0]
            ...
            sx, sxy, sxz = re.findall(
                r"X\(m\)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)"
                r"\s+(-?\d+\.\d+|[nN]a[nN]|\*+)",
                section,
            )[0]
            sy, syz = re.findall(
                r"Y\(m\)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)\s+(-?\d+\.\d+|[nN]a[nN]|\*+)",
                section,
            )[0]
    ...
    def get_pr_observations(self, section, kinematic):
        if self.ppp_version == "1.05":
            processed = re.findall(r"Number of epochs processed\s+\:\s+(\d+)", section)[
                0
            ]
        else:
            processed = re.findall(
                r"Number of epochs processed \(%fix\)\s+\:\s+(\d+)", section
            )[0]
        ...
        if kinematic:
            ...
            rejected = re.findall(
                r"Number of observations rejected\s+\:\s+(\d+)", section
            )
    ...
    def check_otl(section):
        return (
            not len(re.findall(r"Ocean loading coefficients.+NOT FOUND", section)) > 0
        )

    @staticmethod
    def check_eop(section):
        pole = re.findall(
            r"Pole X\s+.\s+(-?\d+\.\d+|[nN]a[nN])\s+(-?\d+\.\d+|[nN]a[nN])", section
        )
    ...
    def parse_summary(self):
        ...
        if len(self.ppp_version) == 0:
            self.ppp_version = re.findall(
                r".*CSRS-PPP ver.\s+(\d.\d+)\/", self.summary
            )[0]
        else:
            self.ppp_version = self.ppp_version[0]

        self.file_summary = self.get_text(self.summary, "SECTION 1.", "SECTION 2.")
        self.proc_parameters = self.get_text(
            self.summary, "SECTION 2. ", " SECTION 3. "
        )
        self.observation_session = self.get_text(
            self.summary, "3.2 Observation Session", "3.3 Coordinate estimates"
        )
        self.coordinate_estimate = self.get_text(
            self.summary, "3.3 Coordinate estimates", "3.4 Coordinate differences ITRF"
        )
        self.clock_estimates = self.get_text(
            self.summary,
        ...
        self.processed_obs, self.rejected_obs = self.get_pr_observations(
            self.observation_session, self.kinematic
        )
        ...
        (
            ...
            self.sigmaxz,
            self.sigmayz,
        ) = self.get_sigmas(self.coordinate_estimate, self.kinematic)
    ...
    def __exec_ppp__(self, raise_error=True):
        try:
            ...
            for i in range(2):
                out, err = pyRunWithRetry.RunCommand(
                    self.ppp, 60, self.rootdir, "input.inp"
                ).run_shell()
                ...
                try:
                    ...
                    if self.observations == OBSERV_CODE_PHASE:
                        ...
                        if not self.kinematic:
                            ...
                            self.rinex.date.fyear >= 2001.33287
                            and not self.clock_interpolation
                        ):
