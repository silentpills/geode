...
from geode.reports.html_utils import escape_report_text
...
def _generate_static_map(plan: dict, width: int = 900, height: int = 420) -> str:
    ...
    try:
        ...
        from staticmap import Line as SLine
        ...
        log.warning(
            "Static map generation failed (%s); print map will be unavailable.", exc
        )
        return ""
...
def _fetch_leaflet() -> tuple:
    ...
    try:
        ...
        js.raise_for_status()
        log.debug(
            "Leaflet assets fetched for inline embedding (%d + %d bytes)",
            len(css.content),
            len(js.content),
        )
        ...
        log.warning(
            "Could not fetch Leaflet assets (%s); falling back to CDN link.", exc
        )
        return None, None
...
def _fmt_time(t: str) -> str:
    ...
    if " +" in t:
        ...
        return (
            f'{hhmm}&thinsp;<sup style="color:#9b9a96;font-size:.75em">{offset}</sup>'
        )
    return t
...
def _stop_rows(day: dict, fuel_per_km: float) -> str:
    ...
    for s in day["stops"]:
        if s.get("type") == "intermediate":
            ...
            warn_style = ""
        drive_cell = (
            f"{_fmt_minutes(s['leg_drive_minutes'])}"
            if s["leg_drive_minutes"]
            else "&mdash;"
        )
        km_cell = f"{s['leg_km']:,.0f}" if s["leg_km"] > 0 else "&mdash;"
        fuel_cell = (
            f"${s['leg_fuel_cost']:.2f}"
            if fuel_per_km and s["leg_km"] > 0
            else "&mdash;"
        )
        ...
        if code and s["name"].lower() != code.lower():
            name_cell = (
                f'{s["name"]} <span style="color:#9b9a96">({code.upper()})</span>'
            )
        else:
            name_cell = s["name"]
        type_labels = {
    ...
    # Totals row
    fuel_total = f"${day['day_total_fuel_cost']:.2f}" if fuel_per_km else "&mdash;"
    rows.append(
...
def _day_sections(plan: dict, fuel_per_km: float) -> str:
    ...
    for day in plan["days"]:
        ...
        heading = (
            f"Day {day['day_number']} &nbsp;&mdash;&nbsp; {_fmt_date_long(day['date'])}"
        )
        dot = (
...
def _cost_summary_html(plan: dict, config: dict) -> str:
    ...
    rows = [
        _row("Driving distance", f"{total_km:,.0f} km", "&mdash;", None),
        _row(
            "Driving time",
    ...
    if fuel_per_km:
        rows.append(
            _row("Fuel", f"{total_km:,.0f} km", f"${fuel_per_km:.4g}/km", fuel_cost)
        )
    ...
    if per_diem_per_day:
        days_label = (
            f"{summary['total_days']} day{'s' if summary['total_days'] != 1 else ''}"
        )
        rows.append(
            _row(
                "Per diem",
...
def generate_html(plan: dict, config: dict) -> str:
    ...
    """
    plan = escape_report_text(plan)
    config = escape_report_text(config)
    summary = plan["summary"]
    fuel_per_km = float(config.get("fuel_cost_per_km", 0.0))
    lodging_per_night = float(config.get("lodging_cost_per_night", 0.0))
    per_diem_per_day = float(config.get("per_diem_cost_per_day", 0.0))
    start_date = config["start_date"]
    end_date_dt = datetime.strptime(start_date, "%Y-%m-%d") + timedelta(
        days=summary["total_days"] - 1
    )
    ...
    fuel_box = (
        ...
        if fuel_per_km
        else '<div class="box"><div class="box-label">Fuel cost</div>'
        '<div class="box-value">N/A</div></div>'
    )
    ...
    lodging_box = (
        ...
        if lodging_per_night
        else '<div class="box"><div class="box-label">Lodging</div>'
        '<div class="box-value">N/A</div></div>'
    )

    per_diem_box = (
        f'<div class="box"><div class="box-label">Per diem</div>'
...
<div class="summary-bar">
  ...
  <div class="box">
    ...
    lodging_total_str = (
        f"${summary['total_lodging_cost']:.2f}" if lodging_per_night else "N/A"
    )
    ...
    static_map_html = (
        f'<img id="map-static" src="{static_map_uri}" alt="Campaign route map">'
        if static_map_uri
        else ""
    )
