...
from . import Utils, pyDate, pyRinexName, pyRunWithRetry
    ...
    def __init__(self, NetworkCode=None, StationCode=None):
        ...
        self.required_records = {
            ...
            "INTERVAL": {
                ...
                "found": False,
                "default": (30,),
            },  # put a wrong interval when first reading the file so that
            ...
            "COMMENT": {"format_tuple": ("%-60s",), "found": True, "default": ("",)},
        }
...
class ReadRinex(RinexRecord):
    ...
    def format_record(self, record_dict, record, values):
        ...
        if len("%-60s" % data) > 60:
            ...
            data = data[0:60]
            self.log_event(
                "Found that record data for "
                + record
                + " was too long (> 60 chars). Replaced with: "
                + data
            )
    ...
    def read_data(self):
        ...
        if not any("END OF HEADER" in s for s in rinex):
            raise pyRinexExceptionBadFile(
                "Invalid header: could not find END OF HEADER tag."
            )
    ...
    def replace_record(self, header, record, new_values):
        ...
        for line in header:
            if line.strip().endswith(record):
                new_header += [
                    self.format_record(self.required_records, record, new_values) + "\n"
                ]
            else:
                new_header += [line]
    ...
    def insert_comment(self, header, comment):
        ...
        new_header = (
            header
            + [self.format_record(self.required_records, "COMMENT", comment) + "\n"]
    ...
    def check_interval(self):
        interval_record = {
            "INTERVAL": {"format_tuple": ("%10.3f",), "found": False, "default": (30,)}
        }
        ...
        for line in self.header:
            if line.strip().endswith("INTERVAL"):
                ...
                fields, _ = self.read_fields(
                    line, "INTERVAL", interval_record["INTERVAL"]["format_tuple"]
                )

                if fields[0] != self.interval:
                    # interval not equal. Replace record
                    new_header += [
                        self.format_record(interval_record, "INTERVAL", self.interval)
                        + "\n"
                    ]
                    self.log_event(
                        "Wrong INTERVAL record, setting to %i" % self.interval
                    )
                else:
    ...
    def check_header(self):
        ...
        for line in self.header:
            ...
            line = "".join(
                i if 32 <= ord(i) < 128 or ord(i) == 10 or ord(i) == 13 else "_"
                for i in line
            )
            ...
            if any(line_strip.endswith(key) for key in self.required_records.keys()):
                ...
                fields, _ = self.read_fields(
                    line, record, self.required_records[record]["format_tuple"]
                )
                ...
                if record == "RINEX VERSION / TYPE":
                    ...
                    if record in ("TIME OF FIRST OBS", "TIME OF LAST OBS"):
                        ...
                        if first_obs is None:
                            ...
                            first_obs = pyDate.Date(
                                year=1990, month=1, day=1, hour=0, minute=0, second=0
                            )

                        # check if the first observation is meaningful or not
                        if record == "TIME OF FIRST OBS" and (
                            fields[0] != first_obs.year
                            or fields[1] != first_obs.month
                            or fields[2] != first_obs.day
                            or fields[3] != first_obs.hour
                            or fields[4] != first_obs.minute
                            or fields[5] != first_obs.second
                        ):
                ...
                # save to new header
                new_header += [
                    self.format_record(self.required_records, record, fields) + "\n"
                ]
            else:
                # not a critical field, just put it back in
                if (
                    not line_strip.endswith("END OF HEADER")
                    and not line_strip.endswith("TIME OF LAST OBS")
                    and line_strip != ""
                ):
                    if line_strip.endswith("COMMENT"):
                        ...
                        fields, _ = self.read_fields(
                            line,
                            "COMMENT",
                            self.required_records["COMMENT"]["format_tuple"],
                        )
                        new_header += [
                            self.format_record(self.required_records, "COMMENT", fields)
                            + "\n"
                        ]
                    else:
                        # leave END OF HEADER until the end to add possible missing records
        ...
        if self.system == "":
            ...
            raise pyRinexExceptionBadFile(
                "Unfixable RINEX header: could not find RINEX VERSION / TYPE"
            )
        ...
        if not all(item["found"] for item in self.required_records.values()):
            ...
            missing_records = {
                item: self.required_records[item]
            ...
            for record in missing_records.keys():
                ...
                new_header += [
                    self.format_record(
                        missing_records, record, missing_records[record]["default"]
                    )
                    + "\n",
                    self.format_record(
                        self.required_records,
    ...
    def indentify_file(self, input_file):
        ...
        self.rinex = self.rinex_name_format.to_rinex_format(
            pyRinexName.TYPE_RINEX, no_path=True
        )
        self.crinez = self.rinex_name_format.to_rinex_format(
            pyRinexName.TYPE_CRINEZ, no_path=True
        )
    ...
    def uncompress(self):
        ...
        if os.path.isfile(self.rinex_path):
            if err and os.path.getsize(self.rinex_path) < size:
                raise pyRinexExceptionBadFile(
                    "Error in ReadRinex.__init__ -- crz2rnx: error and empty file: "
                    + self.origin_file
                    + " -> "
                    + err
                )
            ...
            raise pyRinexException(
                ("Could not create RINEX file. crx2rnx stderr follows: " + err)
                if err
                else "Could not create RINEX file. Unknown reason. Possible problem with crx2rnx?"
            )
    ...
    def ConvertRinex(self, to_version):
        ...
        try:
            ...
            cmd = pyRunWithRetry.RunCommand(
                "gfzrnx_lx -finp %s -fout %s.t -vo %i -f"
                % (self.rinex, self.rinex, to_version),
                45,
                self.rootdir,
            )
            ...
            if "| E |" in err:
                raise pyRinexExceptionBadFile(
                    "gfzrnx_lx returned error converting to RINEX 2.11:\n" + err
                )

            elif not os.path.exists(result_path):
                raise pyRinexExceptionBadFile(
                    "gfzrnx_lx failed to convert to RINEX 2.11:\n" + err
                )
    ...
    def RunRinSum(self):
        ...
        try:
            ...
            cmd = pyRunWithRetry.RunCommand(
                "RinSum --notable " + self.rinex_path, 45
            )  # DDG: increased from 21 to 45.
    ...
    def RunGfzrnx(self):
        ...
        cmd = pyRunWithRetry.RunCommand(
            "gfzrnx_lx -finp %s -fout %s.log -meta medium:json"
    ...
    def __init__(
        ...
        try:
            ...
            raise pyRinexException(
                "File name does not follow the RINEX(Z)/CRINEX(Z) naming convention: %s"
        ...
        if (
            not allow_multiday
            and self.datetime_lastObs.date() != self.datetime_firstObs.date()
        ):
            # more than one day in this file. Is there more than one hour? (at least in principle, based on the time)
            first_obs = datetime.datetime(
                self.datetime_lastObs.date().year,
            ...
            if (self.datetime_lastObs - first_obs).total_seconds() >= 3600:
                ...
                last_obs = datetime.datetime(
                    self.datetime_firstObs.date().year,
                ...
                self.log_event(
                    "RINEX had incomplete epochs (or < 1 hr) outside of the corresponding UTC day -> "
                    "Data windowed to one UTC day."
                )

        # reported date for this file is session/2
        self.date = pyDate.Date(
            datetime=self.datetime_firstObs
            + (self.datetime_lastObs - self.datetime_firstObs) / 2
        )
        ...
        self.rel_completion = (
            ...
            * self.interval
            / (
                (self.datetime_lastObs - self.datetime_firstObs).total_seconds()
                + self.interval
            )
        )
    ...
    def multiday_handle(self, origin_file):
        ...
        if len(self.multiday_rnx_list) > 1:
            ...
            # keep the log
            self.log_event(
                "RINEX appeared to be multiday but had incomplete epochs (or < 1 hr) -> "
                "Data windowed to one UTC day."
            )
            temp_log = self.log
            ...
            # reinitialize self
            self.__init__(
                self.multiday_rnx_list[0].NetworkCode,
    ...
    def split_file(self):
        ...
        cmd = pyRunWithRetry.RunCommand(
            "gfzrnx_lx -finp %s -fout split/::RX%i:: -split 86400 -kv"
            % (self.rinex, int(self.rinex_version)),
            45,
            self.rootdir,
        )
        try:
        ...
        for ff in os.listdir(temp):
            ...
            try:
                rnx = ReadRinex(
                    self.NetworkCode, self.StationCode, os.path.join(self.rootdir, ff)
                )
    ...
    def parse_output(self, output, min_time_seconds=3600):
        ...
        try:
            ...
            self.antOffset, self.antOffsetN, self.antOffsetE = (
                float(p["h"]),
                float(p["n"]),
                float(p["e"]),
            )
        ...
        if self.interval == 0:
            raise pyRinexExceptionSingleEpoch(
                "RINEX interval equal to zero. Single epoch or bad RINEX file. "
                + (
                    ("Reported epochs in file were %i" % self.epochs)
                    if self.epochs > 0
                    else "No epoch information to report. The output from Gfzrnx was:\n"
                    + str(output)
                )
            )
        elif self.interval > 120:
            raise pyRinexExceptionBadFile(
                "RINEX sampling interval > 120s. The output from Gfzrnx was:\n"
                + str(output)
            )

        elif self.epochs * self.interval < min_time_seconds:
            raise pyRinexExceptionBadFile(
                "RINEX file with < %i seconds of observation time. "
        ...
        try:
            ...
            def date2vals(d):
                ...
                dt = datetime.datetime(
                    check_year(yy), int(mm), int(dd), int(hh), int(MM), int(float(ss))
                )
    ...
    def auto_coord(self, brdc, chi_limit=3):
        ...
        from geode import pyOptions, pyPPP
        ...
        ppp = pyPPP.RunPPP(
            ...
            strict=False,
            apply_met=False,
            observations=pyPPP.OBSERV_CODE_ONLY,
        )
        ...
        new_header = self.replace_record(
            self.header, "APPROX POSITION XYZ", (self.x, self.y, self.z)
        )
            ...
            if err != "" and err is not None:
                ...
                for line in out.split("\n"):
                    if "* Final sqrt(chi**2/n)" in line:
                        ...
                        if chi == "NaN":
                            raise pyRinexExceptionNoAutoCoord(
                                "chi2 = NaN! " + str(err) + "\n" + out
                            )

                        elif float(chi) < chi_limit:
                            # open the APR file and read the coordinates
                            apr_path = os.path.join(
                                rnx.rootdir, rnx.rinex[0:4] + ".apr"
                            )
                            if os.path.isfile(apr_path):
                                with file_open(apr_path) as apr:
                                    ...
                                    self.lat, self.lon, self.h = ecef2lla(
    ...
    def decimate(self, decimate_rate, copyto=None):
        ...
        self.log_event(
            "RINEX decimated to %is (applied to %s)" % (decimate_rate, str(copyto))
        )
    ...
    def remove_systems(self, systems=("C", "E", "I", "J", "R", "S"), copyto=None):
        ...
        self.log_event(
            "Removed systems %s (applied to %s)" % (",".join(systems), str(copyto))
        )
    ...
    def normalize_header(self, NewValues=None, brdc=None, x=None, y=None, z=None):
        ...
        if type(NewValues) is StationInfo:
            if NewValues.date is not None and NewValues.date != self.date:
                raise pyRinexException(
                    "The StationInfo object was initialized for a different date than that of the "
                    "RINEX file. Date on RINEX: "
                    + self.date.yyyyddd()
                    + "; Station Info: "
        ...
        if self.marker_name[0:4] != self.StationCode:
            ...
            new_header = self.insert_comment(
                new_header, "PREV MARKER NAME: " + self.marker_name.upper()
            )
        ...
        if (
            NewValues["ReceiverCode"] != self.recType
            or NewValues["ReceiverVers"] != self.recVers
            or NewValues["ReceiverSerial"] != self.recNo
        ):
            new_header = self.replace_record(
                new_header,
                "REC # / TYPE / VERS",
        ...
        if True:
            ...
            if NewValues["AntennaCode"] != self.antType:
                new_header = self.insert_comment(
                    new_header, "PREV ANT #   : " + self.antType
                )
                self.antType = NewValues["AntennaCode"]

            if NewValues["AntennaSerial"] != self.antNo:
                new_header = self.insert_comment(
                    new_header, "PREV ANT TYPE: " + self.antNo
                )
                self.antNo = NewValues["AntennaSerial"]

            if NewValues["RadomeCode"] != self.antDome:
                new_header = self.insert_comment(
                    new_header, "PREV ANT DOME: " + self.antDome
                )
                self.antDome = NewValues["RadomeCode"]

        if (
            NewValues["AntennaHeight"] != self.antOffset
            or NewValues["AntennaNorth"] != self.antOffsetN
            or NewValues["AntennaEast"] != self.antOffsetE
        ):
            new_header = self.replace_record(
                new_header,
                "ANTENNA: DELTA H/E/N",
        ...
        new_header = self.replace_record(
            new_header, "APPROX POSITION XYZ", (self.x, self.y, self.z)
        )

        new_header = self.insert_comment(
            new_header, "APPROX POSITION SET TO AUTONOMOUS SOLUTION"
        )
        new_header = self.insert_comment(
            new_header,
            "HEADER NORMALIZED BY pyRinex ON "
    ...
    def move_origin_file(self, path, destiny_type=TYPE_CRINEZ):
        ...
        if not os.path.isabs(path):
            ...
            path == os.path.dirname(self.origin_file)
            and os.path.basename(self.origin_file) == dst
        ):
            raise pyRinexException(
                "Error during move_origin_file: origin and destiny are the same!"
            )
        ...
        # if other types are requested, or origin is not the destiny type, then use local file and delete the
            ...
            self.create_script(
                "compress.sh", 'cat "%s" | rnx2crx > "%s"' % (self.rinex_path, path_dst)
            )

            # apply hatanaka compression to the destiny file
            cmd = pyRunWithRetry.RunCommand(
                os.path.join(self.rootdir, "compress.sh"), 45
            )
            try:
                _, err = cmd.run_shell()
                ...
                if os.path.getsize(path_dst) == 0:
                    raise pyRinexException(
                        "Error in move_origin_file: compressed version of "
                        + self.rinex_path
                        + " has zero size!\n"
                        + err
                    )
            ...
            raise pyRinexException(
                "pyRinex will not natively generate a RINEZ or CRINEZ_2 (RINEX 2.gz) file."
            )
        ...
        if os.path.isfile(self.origin_file):
            if os.path.isfile(filename):
                ...
                raise pyRinexException(
                    "New 'origin_file' (%s) does not exist!" % os.path.join(path, dst)
                )
        ...
        self.log_event(
            "Origin moved to %s and converted to %i" % (self.origin_file, destiny_type)
        )
    ...
    def compress_local_copyto(self, path, force_filename=None):
        ...
        self.create_script(
            "compress.sh",
            'cat "%s" | rnx2crx | gzip -c > "%s"' % (self.rinex_path, crinez_path),
        )
        ...
        try:
            ...
            if os.path.getsize(crinez_path) == 0:
                raise pyRinexException(
                    "Error in compress_local_copyto: compressed version of "
                    + self.rinex_path
                    + " has zero size!\n"
                    + err
                )
        ...
        filename = Utils.copyfile(
            crinez_path, os.path.join(path, crinez), self.rinex_version
        )
    ...
    def rename(self, new_name=None, NetworkCode=None, StationCode=None):
        ...
        if new_name:
            ...
            if rinex != self.rinex:
                ...
                if os.path.isfile(self.rinex_path):
                    move(self.rinex_path, new_rinex_path)
        ...
        if NetworkCode:
            self.NetworkCode = self.record["NetworkCode"] = NetworkCode.strip().lower()

        if StationCode:
            self.StationCode = self.record["StationCode"] = StationCode.strip().lower()
    ...
    def __add__(self, other):
        if not isinstance(other, ReadRinex):
            raise pyRinexException(
                "type: " + type(other) + " invalid. Can only splice two RINEX objects."
            )
        ...
        cmd = pyRunWithRetry.RunCommand(
            "gfzrnx_lx -finp %s %s -fout %s -vo %i"
            % (
                f1.rinex_path,
                f2.rinex_path,
                f1.rinex_path + ".t",
                int(self.rinex_version * 10) / 10,
            ),
            45,
        )
        # leave errors un-trapped on purpose (will raise an error to the parent)
        ...
        return ReadRinex(
            self.NetworkCode, self.StationCode, filename, allow_multiday=True
        )

    def __repr__(self):
        return "pyRinex.ReadRinex(%s, %s, %s, %s)" % (
            self.NetworkCode,
            self.StationCode,
            str(self.date.year),
            str(self.date.doy),
