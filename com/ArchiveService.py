    ...
    if rs.ntuples():
        ...
        while (
            ...
            if index > 255:
                ...
                raise Exception(
                    "While looking for a temporary network code, "
...
def callback_handle(job):
    ...
    if job.result is not None:
        ...
        if new_station:
            ...
            tqdm.write(
                " -- New station %s was found in the repository at %s. "
                "Please assign a network to the new station and remove "
                "the locks from the files before running again ArchiveService."
...
def error_handle(cnn, event, crinez, folder, filename, no_db_log=False):
    ...
    try:
        ...
        message = (
            "could not move file into this folder!"
            + str(e)
            + "\n. Original error: "
            + event["Description"]
        )
...
def process_crinex_file(crinez, filename, data_rejected, data_retry):
    ...
    try:
        ...
        with pyRinex.ReadRinex(NetworkCode, StationCode, crinez) as rinexinfo:
            ...
            with pyPPP.RunPPP(
                ...
                if result:
                    ...
                    error = (
                        "%s matches the coordinate of %s.%s (distance = %8.3f m) but the filename "
                        ...
                        os.path.join(
                            retry_folder,
                            filename.replace(StationCode, match[0]["StationCode"]),
                        ),
                        StationCode,
                        ppp.x,
                        ppp.y,
                    ...
                    error = (
                        "Solution for RINEX in repository (%s %s) did not match a unique station location "
                    ...
                    cnn.insert(
                        "locks",
                        filename=os.path.relpath(crinez, Config.repository_data_in),
                    )
                    cnn.close()
                    return (
                        None,
        ...
        pyRinex.pyRinexExceptionNoAutoCoord,
    ) as e:
        ...
        fill_event(
            e.event,
            "\n%s: (file moved to %s)"
            % (os.path.relpath(crinez, Config.repository_data_in), reject_folder),
        )
        ...
        fill_event(
            e.event,
            ". The file will stay in the repository and will "
        ...
        fill_event(
            e.event,
            " while calculating OTL for %s. The file has been moved into the retry folder."
            % os.path.relpath(crinez, Config.repository_data_in),
        )
        error_handle(cnn, e.event, crinez, retry_folder, filename)
        ...
        fill_event(
            e.event,
            " during %s. The file has been moved to the rejected folder. "
        ...
        fill_event(
            e.event,
            ": %s. Check the brdc/sp3/clk files and also check that the RINEX data is not corrupt."
            % os.path.relpath(crinez, Config.repository_data_in),
        )
...
def print_archive_service_summary():
    ...
    if station_import_counts:
        ...
        print(" -- " + "-" * 32)
        for stn, count in sorted(
            station_import_counts.items(), key=lambda x: (-x[1], x[0])
        ):
    ...
    try:
        ...
        if result:
            event = pyEvents.Event(
                ...
                "RINEX and left in the repository" % record["filename"],
            )
            cnn.insert_event(event)
            # mark the file as done
            cnn.update(
                "api_visitgnssdatafiles", {"rinexed": True}, id=record["file_id"]
            )
        else:
            event = pyEvents.Event(
                NetworkCode=record["NetworkCode"],
...
def process_visits(JobServer):
    ...
    stns = cnn.query_float(
        """SELECT "NetworkCode", "StationCode" FROM api_visitgnssdatafiles 
    ...
    # now get visit files
    rs = cnn.query_float(
        """SELECT api_visitgnssdatafiles.id as file_id, * FROM api_visitgnssdatafiles 
                                     LEFT JOIN api_visits ON visit_id = api_visits.id 
                                     LEFT JOIN stations   ON station_id = stations.api_id 
                                     WHERE rinexed = False
                                     """,
    ...
    pbar = tqdm(
        desc="%-30s" % " >> Processing visits", total=len(rs), ncols=160, disable=None
    )
    ...
    pbar = tqdm(
        desc="%-30s"
        % " >> Merging RINEX files (same day, same interval, may take a while!)",
        total=len(stns),
        ncols=160,
        disable=None,
    )
...
def main():
    ...
    if args.purge_locks:
        ...
        for lock in tqdm(
            ...
            file_try_remove(os.path.join(Config.repository_data_in, lock["filename"]))
    ...
    pbar = tqdm(
        desc="%-30s" % " >> Scanning data_in_retry", ncols=160, unit="crz", disable=None
    )
    ...
    pbar = tqdm(
        desc="%-30s" % " -- Moving files to data_in",
        total=len(rfiles),
        ncols=160,
        unit="crz",
        disable=None,
    ...
    for rfile, path in zip(rfiles, paths):
        ...
        file_try_remove(pyRinexName.RinexNameFormat(path).filename_no_ext() + ".log")
    ...
    pbar = tqdm(
        desc="%-30s" % " -- Checking the locks table",
        total=len(files),
        ncols=130,
        unit="crz",
        disable=None,
    ...
    tqdm.write(
        " -- Found %i files (matching RINEX 2/3 format) to process..." % len(files_list)
    )

    pbar = tqdm(
        desc="%-30s" % " >> Processing repository",
    ...
    # dependency functions
    depfuncs = (
        check_rinex_timespan_int,
        write_error,
        error_handle,
        insert_data,
        verify_rinex_multiday,
        file_append,
        file_try_remove,
        file_open,
    )
