...
from geode.etm.core.data_classes import (
    AdjustmentModels,
    CovarianceFunction,
    JumpParameters,
    ModelingParameters,
...
def process_etm_station(stn, args_dict, from_kmz):
    ...
    from geode.etm.etm_functions.polynomial import PolynomialFunction
    from geode.pyDate import Date
    from geode.Utils import lg2ct, load_json, process_date_str
    ...
    try:
        ...
        if not args_dict["filename"]:
            solution_options.solution_type = SolutionType.from_code(
                args_dict["solution"]
            )
        ...
        config.plotting_config.plot_residuals_mode = (
            "residuals" in args_dict["plot_options"]
        )
        config.plotting_config.plot_missing_solutions = (
            "missing" in args_dict["plot_options"]
        )
            ...
            while i < len(prefit_args):
                if cur == "jump":
                    ...
                    jf = [
                        f
                        for f in funcs
                        if f["object"] == fmap_str[cur] and f["jump_date"]["mjd"] == mjd
                    ]
                    fn = fmap[cur](
                        config,
        ...
        config.modeling.least_squares_strategy.adjustment_model = ADJUSTMENT_MODEL_MAP[
            args_dict["least_squares_strategy"]
        ]
        config.modeling.fit_auto_detected_jumps = (
            args_dict["fit_auto_jumps"][0] is not None
        )
        ...
        config.modeling.least_squares_strategy.sigma_filter_limit = args_dict[
            "sigma_limit"
        ]
        config.modeling.least_squares_strategy.covariance_function = COVARIANCE_MAP[
            args_dict["covariance"][0]
        ]
        ...
        for event, relax in events:
            rs = cnn.query_float(
                "SELECT * FROM earthquakes WHERE id = '%s'" % event, as_dict=True
            )[0]
        ...
        etm.run_adjustment(
            cnn=cnn, try_save_to_db=not no_save_db, try_loading_db=not no_save_db
        )

        json_opts = args_dict["json"]
        if json_opts is not None:
            etm.save_etm(
                filename=args_dict["directory"],
        ...
        if args_dict["query"]:
                ...
                if "per" in args_dict["query"]:
                    ...
                    for x in etm.design_matrix.functions:
                        if x.p.object == "periodic":
                            for k in range(3):
                                pxyz += [
                                    "%8.5f" % (p * 1000) for p in x.p.params[k].tolist()
                                ]
                    strp += " ".join(pxyz)
            ...
            for i, d in enumerate(q_date):
                ...
                if extrapolate or (
                    etm.config.metadata.first_obs < d < etm.config.metadata.last_obs
                ):
                    buf.write(
                        " %s %14.5f %14.5f %14.5f %8.3f %s -> %s\n"
                        % (
                            sid,
                    ...
                    buf.write(
                        " %s %14s %14s %14s %8.3f %s -> %s\n"
                        % (sid, "NaN", "NaN", "NaN", d.fyear, strp, solution["source"])
                    )
...
def read_kml_or_kmz(
    ...
    for stnm, coord in placemarks:
        ...
        if cnn:
            ...
            filename = filenames.replace("{net}", network_code).replace(
                "{stn}", station_code
            )
        ...
        if metadata:
            stn = StationInfo(
                cnn=None, NetworkCode=network_code, StationCode=station_code
            )
            stninfo = [
                item.to_json()
                for item in stn.parse_station_info(metadata)
        ...
        stnlist.append(
            {
                ...
                "modeling_params": asdict(
                    ModelingParameters(earthquake_jumps=score_table)
                ),
        ...
        for coord in placemark.findall(".//kml:coordinates", ns):
            ...
            for coord_str in coords:
                lon, lat, height = coord_str.split(
                    ","
                )  # Ignore the altitude (third value)
                coordinates_list.append(
                    [
                        np.array([float(lat)]),
                        np.array([float(lon)]),
                        np.array([float(height)]),
                    ]
                )
...
def process_plot_dates(args):
    ...
    if args.plot_window:
        if len(args.plot_window) == 1:
            try:
                dates = process_date(
                    args.plot_window, missing_input=None, allow_days=False
                )
                dates = (dates[0].fyear,)
            except ValueError:
                # an integer value
...
def process_fit_dates(args):
    ...
    if args.fit_window is not None:
        ...
        for i in range(0, len(args.fit_window), 2):
            try:
                dates.append(
                    [d.fyear for d in process_date(args.fit_window[i : i + 2])]
                )
            except ValueError:
        ...
        while i < len(args):
            ...
            if current_funct == "jump":
                ...
                jf = [
                    ...
                    if funct["object"] == function_map_str[current_funct]
                    and funct["jump_date"]["mjd"] == mjd
                ]
                # create instance of function
                funct = function_map[current_funct](
                    config,
                    time_vector=np.array([0]),
                    date=Date(**jf[0]["jump_date"]),
                    jump_type=JumpType.UNDETERMINED,
            ...
            if not len(jf):
                raise ValueError(
                    "Constraint function "
                    + function_map_str[current_funct]
    ...
    if args:
        ...
        while i < len(args):
            ...
            if arg_count == 0:
                # find the json info for this function
                print(
                    " -- Adding " + function_map_str[current_funct] + " as constraint"
                )
                if current_funct == "jump":
                    # if it is a jump assimilate the date also
                    mjd = process_date_str(args[i]).mjd
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot extended trajectory models (ETMs) "
    ...
    parser.add_argument(
        ...
        type=float,
        default=ValidationRules().max_condition_number,
        metavar="log10(cond)",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        metavar="event_id relax1 relax2",
    ...
    if args.stnlist[0].endswith(("kmz", "kml")):
        ...
        stnlist = read_kml_or_kmz(
            cnn,
            args.stnlist[0],
            args.metadata_filename,
            args.s_score_mag_limit,
            args.filename,
            args.format,
        )
    ...
    JobServer = pyJobServer.JobServer(
        Config, check_archive=False, check_executables=False, check_atx=False
    )

    pbar = tqdm(
        desc="%-30s" % " >> Processing ETM", total=len(stnlist), ncols=160, disable=None
    )
