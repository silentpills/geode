...
from geode.reports.station_kmz import build_kmz, station_data_from_db
...
def main():
    ...
    if not args.stnlist and not args.project:
        parser.error(
            "Provide at least one station (stnlist) or a project file (--project)."
        )

    # ── Database connection ───────────────────────────────────────────────────
    cnn = dbConnection.Cnn(get_gnss_data_cfg_path(), write_cfg_file=True)

    cfg = configparser.ConfigParser()
    cfg.read(get_gnss_data_cfg_path())
    media_path = os.getenv("MEDIA_FOLDER_HOST_PATH") or cfg.get(
        "archive", "media", fallback=None
    )
    ...
    if args.project:
        ...
        project_stn_set = {stationID(s) for s in process_stnlist(cnn, proj_raw)}
        print(
            f" >> Project: {project_name} ({len(project_stn_set)} stations)",
            file=sys.stderr,
        )
