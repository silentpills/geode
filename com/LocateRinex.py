...
def main():
    parser = argparse.ArgumentParser(
        description="Simple PPP python wrapper. Calculate a coordinate for a RINEX file. "
...
def execute_ppp(
    ...
    try:
        ...
        rinexinfo.normalize_header(
            stninfo
        )  # empty dict: only applies the coordinate change
    except pyRinex.pyRinexException as e:
        print(str(e))
    ...
    try:
        ...
        if args.ocean_loading or args.insert_sql:
            ...
            otl = pyOTL.OceanLoading(
                stnm, options["grdtab"], options["otlgrid"], ppp.x, ppp.y, ppp.z
            )
        ...
        if not args.insert_sql:
            print(
                ...
                % (
                    ...
                    ppp.h[0],
                    ppp.clock_phase,
                    ppp.clock_phase_sigma,
                    ppp.phase_drift,
                    ppp.phase_drift_sigma,
                    ppp.clock_rms,
                )
            )
        else:
            ...
            if args.insert_sql == "???":
                ...
                try:
                    ...
                    print(
                        "Station %s.%s added to the database (country code: %s)"
                        % (NetworkCode, stnm, ISO3)
                    )
                except dbConnection.dbErrInsert:
                    print(
                        "Station %s.%s (country code: %s) already exists in database"
                        % (NetworkCode, stnm, ISO3)
                    )
                    pass
        ...
        if args.find:
            ...
            if Result:
                ...
                print(
                    "Solution for RINEX (%s %s) did not match a unique station location (and station code) "
                ...
                print(
                    "No matches found. Closest station: %s.%s. (distance = %8.3f m)"
                    % (
        ...
        if copy_results:
            ...
            try:
                ...
                shutil.copyfile(
                    ...
                    os.path.join(fpath, os.path.basename(ppp.path_sum_file) + ".cmd"),
                )
