...
def main():
    ...
    GamitConfig = GamitConfiguration(args.project_file[0], check_config=False)

    # global variable that contains the station list in the project
    stnlist = [
        stationID(s)
        for s in process_stnlist(
            cnn,
    ...
    stnonly = [
        stationID(s)
        for s in process_stnlist(
            cnn, args.station_list, summary_title="Stations requested:"
        )
    ]
...
def description_content(
    ...
    %s
        ...
        count,
        completion,
        "<br>".join(stn_issues) if stn_issues else ("NO ISSUES FOUND OR NOT REQUESTED"),
        stninfo,
        plot_station_info_rinex(
            cnn, stn["NetworkCode"], stn["StationCode"], stninfo_records
        ),
        data_plt,
    )
...
def generate_kml_stninfo(
    ...
    if stnonly:
        rs = cnn.query_float(
            ...
            'ORDER BY "NetworkCode", "StationCode" ' % "','".join(stnonly),
            as_dict=True,
        )
    else:
        rs = cnn.query_float(
    ...
    for stn in rs:
        count = cnn.query_float(
            """SELECT count(*) as cc FROM rinex_proc
                                WHERE "NetworkCode" = \'%s\'
                                AND "StationCode" = \'%s\' """
        ...
        ppp_s = cnn.query_float(
            """SELECT count(*) as cc FROM ppp_soln
                                WHERE "NetworkCode" = \'%s\'
                                AND "StationCode" = \'%s\' """
            % (stn["NetworkCode"], stn["StationCode"])
        )[0][0]
...
def generate_kml(cnn, project, data=False):
    ...
    for stn in tqdm(rs, ncols=80):
        ...
        if stn_id in stnlist:
            ...
            if (
                count[0][0] >= 100
                and (float(ppp_s[0][0]) / float(count[0][0]) * 100) >= 60.0
            ):
                style = styles_ok
            else:
                style = styles_nok
        ...
        plt = plot_station_info_rinex(
            cnn, stn["NetworkCode"], stn["StationCode"], stninfo
        )
