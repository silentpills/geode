...
from geode.etm.core.etm_stacker import ConstraintType, EtmStacker, EtmStackerConfig
...
class EtmStackerShell(cmd.Cmd):
    ...
    def do_add_event(self, arg):
        ...
        for e in arg.split():
            # find the event id
            j = self.cnn.query_float(
                f"SELECT * FROM earthquakes WHERE id = '{e}'", as_dict=True
            )

            if len(j) > 0:
                j = j[0]
                strike = (
                    [float(j["strike1"]), float(j["strike2"])]
                    if not math.isnan(j["strike1"])
                    else []
                )
                dip = (
                    [float(j["dip1"]), float(j["dip2"])]
                    if not math.isnan(j["strike1"])
                    else []
                )
                rake = (
                    [float(j["rake1"]), float(j["rake2"])]
                    if not math.isnan(j["strike1"])
                    else []
                )
    ...
    def do_list(self, arg):
        ...
        if arg == "events":
            ...
            for station in self.etm_stacker.stations:
                print(
                    f"{stationID(station)} weight {station.normal_equations.weight_scale}"
                )
    ...
    def do_update_constraint(self, arg):
        ...
        if not arg:
            print(
                "syntax: update_constraint <interseismic|coseismic|postseismic> [event_id] [relax] "
        ...
        if parts[0] not in ("interseismic", "coseismic", "postseismic"):
            print(
                f"{parts[0]}: invalid constraint type, use interseismic, coseismic, or postseismic"
            )
            return
        ...
        try:
            if len(parts) == 5:
                ...
                relax = float(parts[2])
                h_sigma = float(parts[3]) / 1000.0
                v_sigma = float(parts[4]) / 1000.0
                ...
                # type event_id h v
                h_sigma = float(parts[2]) / 1000.0
                v_sigma = float(parts[3]) / 1000.0
                self.etm_stacker.update_weights(
                    constraint_type=parts[0],
                    event_id=parts[1],
                    h_sigma=h_sigma,
                ...
                # type h v
                h_sigma = float(parts[1]) / 1000.0
                v_sigma = float(parts[2]) / 1000.0
                self.etm_stacker.update_weights(
    ...
    def do_okada_weights(self, arg):
        ...
        n = self.etm_stacker.update_okada_weights(
            h_weight, v_weight, event_id=event_id, relax=relax
        )
        if n == 0:
            return
    ...
    def do_print(self, arg=""):
        ...
        if self.etm_stacker.solved and arg == "rms":
            ...
            for item in wrms:
                ...
                print(
                    f"{item[0].short_description():60s}" + f" {item[1]:4} equations -> "
                    f"rms: {item[2] * 1000.0: 8.3f} " + unit + " "
                    f"a priori: {item[0].h_sigma * 1000.0:.3f} {item[0].v_sigma * 1000.0:.3f}"
                )
            ...
            print(
                f"{header[0]:8s} {header[1]:15s} {header[2]:15s} {header[3]:27s} "
            ...
            for stn in self.velocities:
                ...
                print(
                    f"{name} {lon:15.8f} {lat:15.8f} {vp[0] * 1000.0:9.2f} "
                    f"{vp[1] * 1000.0:9.2f} {vp[2] * 1000.0:9.2f} {vc[0] * 1000.0:9.2f} "
                    f"{vc[1] * 1000.0:9.2f} {vc[2] * 1000.0:9.2f} {inter}"
                )
        ...
        if arg:
            ...
            for stn in stnlist:
                ...
                try:
                    ...
                    etm.fit.outlier_flags[etm.solution_data.time_vector < 1995.0] = (
                        False
                    )
                    ...
                    for field in self.etm_stacker.fields:
                        ...
                        if (
                            field.base_type != ConstraintType.INTERSEISMIC
                            and field.onset_date.fyear
                            > etm.solution_data.time_vector.max()
                        ):
                            # time series stopped, do not evaluate any more fields
                            break

                        m = field.eval(
                            config.metadata.lon[0],
                            config.metadata.lat[0],
                    ...
                    stn_t.extend(
                        etm.solution_data.time_vector[etm.fit.outlier_flags].tolist()
                    )
                    stn_e.extend(enu_[0].tolist())
                    stn_n.extend(enu_[1].tolist())
                    stn_u.extend(enu_[2].tolist())

                    prgn = (enu.max(axis=1) - enu.min(axis=1)) * 1000.0
                    argn = (enu_.max(axis=1) - enu_.min(axis=1)) * 1000.0

                    print(
                        f"{stationID(stn)} value range PRIOR/AFTER [mm] "
            ...
            if arg.split()[0] == "ts":
                self.plot_time_series(
                    np.array(stn_t),
                    np.array(stn_e) * 1000.0,
                    np.array(stn_n) * 1000.0,
                    np.array(stn_u) * 1000.0,
                )
            else:
                self.plot_darts(
                    np.array(stn_t),
    ...
    def do_overlay(self, arg):
        ...
        if arg:
            ...
            for stn in stnlist:
                ...
                etm.fit.outlier_flags[etm.solution_data.time_vector < 1995.0] = False
                ...
                for field in self.etm_stacker.fields:
                    ...
                    if (
                        field.base_type != ConstraintType.INTERSEISMIC
                        and field.onset_date.fyear > etm.solution_data.time_vector.max()
                    ):
                        # time series stopped, do not evaluate any more fields
                        break
    ...
    def plot_darts(t_, e_, n_, u_):
        ...
        print(
            f"Filtered {len(mask) - np.sum(mask)} observations as outliers out of {len(mask)} "
        )
        ...
        ax.set_title("Residuals East-North")
        ax.axis("equal")
        ...
        ax.hist(
            ...
            alpha=0.75,
            density=True,
            color=(0, 150 / 255, 235 / 255),
            orientation="horizontal",
        )

        ax.set_xlabel("Frequency")
        ax.set_ylabel("North Residuals [mm]")
        ax.set_ylim(ylim)
        ax.grid(True)
        ...
        ax.set_xlabel("Frequency")
        ax.set_ylabel("East Residuals [mm]")
        ax.grid(True)
        ax.set_xlim(xlim)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Routines to stack ETMs and jointly estimate parameters",
        formatter_class=argparse.RawTextHelpFormatter,
    )

    parser.add_argument("stnlist", type=str, nargs="*", help=station_list_help())
    ...
    parser.add_argument(
        ...
        type=float,
        default=[
            def_config.interseismic_h_sigma * 1000.0,
            def_config.interseismic_v_sigma * 1000.0,
        ],
        metavar="mm/yr",
        nargs=2,
        help=f"Interseismic horizontal and vertical sigmas (given in mm/yr) to use for the "
        f"interseismic constraints. Default h={def_config.interseismic_h_sigma * 1000.0} "
        f"mm/yr and v={def_config.interseismic_v_sigma * 1000.0} mm/yr.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=[
            def_config.coseismic_h_sigma * 1000.0,
            def_config.coseismic_v_sigma * 1000.0,
        ],
        metavar="mm",
        nargs=2,
        help=f"Coseismic horizontal and vertical sigmas (given in mm) to use for the "
        f"coseismic constraints. Default values are h={def_config.coseismic_h_sigma * 1000.0} "
        f"mm and v={def_config.coseismic_v_sigma * 1000.0} mm.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=[
            def_config.postseismic_h_sigma * 1000.0,
            def_config.postseismic_v_sigma * 1000.0,
        ],
        metavar="mm",
        nargs=2,
        help=f"Postseismic horizontal and vertical sigmas (given in mm) to use for the "
        f"postseismic constraints. Default values are h={def_config.postseismic_h_sigma * 1000.0} "
        f"mm and v={def_config.postseismic_v_sigma * 1000.0} mm.",
    )
    ...
    parser.add_argument(
        ...
        "--vertical_method",
        type=str,
        default=def_config.vertical_method,
        metavar="method",
        choices=["diskload", "spline2d", "rectload"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=def_config.vertical_load_radius,
        metavar="km",
    ...
    if not args.load_from_file:
        config = EtmStackerConfig(
            ...
            interseismic_h_sigma=args.interseismic_sigmas[0] / 1000.0,
            interseismic_v_sigma=args.interseismic_sigmas[1] / 1000.0,
            coseismic_h_sigma=args.coseismic_sigmas[0] / 1000.0,
            coseismic_v_sigma=args.coseismic_sigmas[1] / 1000.0,
            postseismic_h_sigma=args.postseismic_sigmas[0] / 1000.0,
            postseismic_v_sigma=args.postseismic_sigmas[1] / 1000.0,
        ...
        etm_stacker.filename = os.path.basename(args.load_from_file).removesuffix(
            ".pkl"
        )
