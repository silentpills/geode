...
def callback_handle(job):
    ...
    if job.result is not None or job.exception:
        ...
        tqdm.write(
            " -- There were unhandled errors during this batch. "
...
def try_insert(NetworkCode, StationCode, year, doy, rinex):
    try:
        ...
        data_reject = os.path.join(
            Config.repository_data_reject, "bad_rinex/%i/%03i" % (year, doy)
        )
        ...
        return (
            ...
            + " processing rinex: %s (%s.%s %s %s) using node %s"
    ...
    try:
            ...
            with pyRinex.ReadRinex(NetworkCode, StationCode, rinex) as rinexinfo:
                ...
                if verify_rinex_date_multiday(cnn, date, rinexinfo, Config):
                    try:
                        ...
                        event = pyEvents.Event(
                            Description="Archived crinex file %s added to the database."
                            % (rinex),
                            EventType="info",
                        ...
                        event = pyEvents.Event(
                            ...
                            "it matched the interval and completion of an already existing file."
                            % rinex,
                            EventType="info",
                            StationCode=StationCode,
                            NetworkCode=NetworkCode,
                        ...
                        rinexinfo.move_origin_file(
                            os.path.join(
                                Config.repository_data_reject,
        ...
        try:
            filename = Utils.move(
                rinex, os.path.join(data_reject, os.path.basename(rinex))
            )
        except OSError:
...
def obtain_otl(NetworkCode, StationCode):
    ...
    try:
        ...
        count = cnn.query(
            "SELECT count(*) as cc FROM rinex_proc as r WHERE \"NetworkCode\" = '%s' AND "
        ...
        if count[0]["cc"] >= 10:
            ...
            stn = cnn.query(
                "SELECT * FROM (SELECT row_number() OVER() as rnum, r.* FROM rinex_proc as r "
            ...
            return (
                "Station %s.%s had no rinex files in the archive. Please check the database for problems."
                % (NetworkCode, StationCode)
            )
        ...
        for dbRinex in tblrinex:
            ...
            file = pyArchive.build_rinex_path(
                NetworkCode,
                StationCode,
                ...
                try:
                    ...
                    try:
                        brdc = pyProducts.GetBrdcOrbits(
                            Config.brdc_path, Rinex.date, Rinex.rootdir
                        )
                    ...
                    return traceback.format_exc() + " processing: %s using node %s" % (
                        stn_id,
                        platform.node(),
                    )
        ...
        if len(x) > 0:
            ...
            otl = pyOTL.OceanLoading(
                StationCode, Config.options["grdtab"], Config.options["otlgrid"]
            )
            coeff = otl.calculate_otl_coeff(x=x, y=y, z=z)

            # update record in the database
            cnn.query(
                'UPDATE stations SET "auto_x" = %.3f, "auto_y" = %.3f, "auto_z" = %.3f, '
            ...
            outmsg = (
                "Could not obtain a coordinate/otl coefficients for "
                + NetworkCode
                + " "
                + StationCode
        ...
        return (
            "Error while calculating OTL for %s: %s\n" % (stn_id, str(e))
            + "Debug info and errors follow: \n"
            + errors
        )
...
def execute_ppp(record, rinex_path, h_tolerance):
    ...
    try:
        ...
        frame, _ = Utils.determine_frame(
            Config.options["frames"], pyDate.Date(year=year, doy=doy)
        )
        ...
        if ppp_soln.ntuples() == 0:
            ...
            with pyRinex.ReadRinex(NetworkCode, StationCode, rinex_path) as Rinex:
                if not verify_rinex_date_multiday(cnn, Rinex.date, Rinex, Config):
                    ...
                    cnn.query(
                        "DELETE FROM rinex WHERE %s AND \"Filename\" = '%s'"
                        % (where_obs, record["Filename"])
                    )
                    cnn.commit_transac()
                    cnn.close()
                    return
                ...
                Rinex.normalize_header(
                    stninfo, x=stn[0]["auto_x"], y=stn[0]["auto_y"], z=stn[0]["auto_z"]
                )
                ...
                with pyPPP.RunPPP(
                    ...
                    hash=stninfo.current_record.hash,
                ) as ppp:
                    ppp.exec_ppp()
                    ...
                    if Result and (
                        match[0]["NetworkCode"] == NetworkCode
        ...
        return (
            traceback.format_exc()
            + " processing rinex: %s.%s %s %s using node %s"
            % (NetworkCode, StationCode, str(year), str(doy), platform.node())
        )


def post_scan_rinex_job(
    cnn, Archive, rinex_file, rinexpath, master_list, JobServer, ignore
):
    valid, result = Archive.parse_archive_keys(
        rinex_file, key_filter=("network", "station", "year", "doy")
    )
    ...
    if valid:
        ...
        if NetworkCode + "." + StationCode in master_list or ignore:
            ...
            rs = cnn.query(
                "SELECT * FROM networks WHERE \"NetworkCode\" = '%s'" % NetworkCode
            )
            if rs.ntuples() == 0:
...
def scan_rinex(cnn, JobServer, pyArchive, archive_path, master_list, ignore):
    ...
    JobServer.create_cluster(
        try_insert, dependencies=depfuncs, modules=modules, callback=callback_handle
    )
    ...
    for path, _, files in os.walk(archive_path):
        for sfile in files:
            ...
            try:
                ...
                post_scan_rinex_job(
                    cnn, pyArchive, rnx, path2rnx, master_list, JobServer, ignore
                )
    ...
    if error_message:
        tqdm.write(
            " -- There were unhandled errors. Please check %s for details" % ERRORS_LOG
        )
...
def process_otl(cnn, JobServer, master_list):
    ...
    JobServer.create_cluster(
        obtain_otl, depfuncs, callback_handle, progress_bar=pbar, modules=modules
    )

    for record in records:
        JobServer.submit(record["NetworkCode"], record["StationCode"])
...
def scan_station_info(JobServer, pyArchive, archive_path, master_list):
    ...
    JobServer.create_cluster(
        insert_stninfo, callback=callback_handle, progress_bar=pbar, modules=modules
    )

    for stninfofile, stninfopath in zip(stninfo, path2stninfo):
        valid, result = pyArchive.parse_archive_keys(
            stninfofile, key_filter=("network", "station")
        )
...
def scan_station_info_man(
    cnn, pyArchive, stn_info_path, stations, stn_info_net, stdin=None
):
    ...
    def process_stations(stninfo_file_list, desc):
        ...
        for Station in tqdm(stations, total=len(stations), disable=None):
            ...
            if Station["StationCode"] in stn_codes:
                tqdm.write(
                    "   >> Processing %s using network code %s"
                    % (Station["StationCode"], NetworkCode)
                )
                out = insert_stninfo(
                    NetworkCode, Station["StationCode"], stninfo_file_list
                )
                ...
                tqdm.write(
                    "   >> Station %s.%s was not found in the station info file %s"
...
def hash_check(cnn, master_list, sdate, edate, rehash=False, h_tolerant=0):
    ...
    master_list = [
        item["NetworkCode"] + "." + item["StationCode"] for item in master_list
    ]

    ppp_soln = cnn.query(
        "SELECT * FROM ppp_soln "
        'WHERE "NetworkCode" || \'.\' || "StationCode" IN (\''
        + "','".join(master_list)
        + "') "
    ...
    for soln in tqdm(tbl, ncols=80, disable=None):
        ...
        try:
            ...
            rinex = archive.get_rinex_record(
                NetworkCode=soln["NetworkCode"],
            ...
            if not rinex:
                ...
                tqdm.write(
                    " -- Could not find RINEX for %s. PPP solution will be deleted."
                    % obs_id
                )
                ...
                dd = (
                    rinex["ObservationSTime"]
                    + (rinex["ObservationETime"] - rinex["ObservationSTime"]) // 2
                )
                ...
                if stninfo.current_record.hash + crc32(soln["orbit"]) != soln["hash"]:
                    if not rehash:
                        tqdm.write(
                            " -- Hash value for %s does not match with Station Information hash. "
...
def process_ppp(
    ...
    rs_rnx = cnn.query(
        "SELECT rinex.* FROM rinex_proc as rinex "
        ...
        'rinex."NetworkCode" || \'.\' || rinex."StationCode" IN (\''
        + "','".join(master_list)
        + "') "
    ...
    for record in tblrinex:
        rinex_path = pyArchive.build_rinex_path(
            record["NetworkCode"],
            record["StationCode"],
            record["ObservationYear"],
            record["ObservationDOY"],
...
def print_scan_archive_summary(cnn):
    ...
    exec_date = cnn.query_float(
        "SELECT max(exec_date) as mx FROM executions WHERE script = 'ScanArchive.py'"
    )

    info = cnn.query_float(
        "SELECT count(*) as cc FROM events WHERE \"EventDate\" >= '%s' AND \"EventType\" = 'info'"
        % exec_date[0][0]
    )

    erro = cnn.query_float(
        "SELECT count(*) as cc FROM events WHERE \"EventDate\" >= '%s' AND \"EventType\" = 'error'"
        % exec_date[0][0]
    )

    warn = cnn.query_float(
        "SELECT count(*) as cc FROM events WHERE \"EventDate\" >= '%s' AND \"EventType\" = 'warn'"
        % exec_date[0][0]
    )
...
def export_station(cnn, stnlist, pyArchive, archive_path, dataless):
    ...
    for stn in stnlist:
        ...
        if stn["lat"] and stn["auto_x"] and stn["Harpos_coeff_otl"]:
            ...
            tqdm.write(
                " -- Warning! %s.%s has incomplete station data"
                % (NetworkCode, StationCode)
            )
        ...
        rinex_dict = []
        pbar2 = tqdm(
            total=len(rinex_lst), ncols=160, position=1, disable=None, leave=False
        )

        tqdm.write(
            " >> Creating station package for %s.%s" % (NetworkCode, StationCode)
        )

        with zipfile.ZipFile(
            "%s.%s.zip" % (NetworkCode, StationCode),
            "w",
            zipfile.ZIP_DEFLATED,
            allowZip64=True,
        ) as zf:
            for rnx in rinex_lst:
                # update bar first to provide more information
                pbar2.set_postfix(
                    Date="%4.0f %03.0f"
                    % (rnx["ObservationYear"], rnx["ObservationDOY"])
                )
                pbar2.update()
                # make a copy of each file
                rnx_path = pyArchive.build_rinex_path(
                    NetworkCode=NetworkCode,
                    StationCode=StationCode,
                ...
                # tqdm.write(' -- Adding %s observation date %4.0f %03.0f filename %s'
            ...
            with zf.open("%s.%s.json" % (NetworkCode, StationCode), "w") as json_file:
                data_bytes = json.dumps(
                    export_dic, indent=4, sort_keys=True, cls=Encoder
                ).encode("utf-8")
        ...
        if filename.endswith(".zip"):
            ...
            try:
                ...
                rnx_files = [
                    f
                    for f_ in [
                        glob.glob(os.path.join(path, e)) for e in ("*.gz", "*d.Z")
                    ]
                    for f in f_
                ]
                ...
                spatial = pyPPP.PPPSpatialCheck(
                    [station["lat"]], [station["lon"]], [station["height"]]
                )

                result, match, closest_stn = spatial.verify_spatial_coherence(
                    cnn, StationCode
                )
                ...
                if result:
                    tqdm.write(
                        " -- Found external station %s.%s in network %s (distance %.3f)"
                        % (
                            NetworkCode,
                            StationCode,
                            match[0]["NetworkCode"],
                    ...
                    tqdm.write(
                        " -- External station %s.%s not found. Possible match is %s.%s: %.3f m"
                        % (
                            NetworkCode,
                            StationCode,
                            match[0]["NetworkCode"],
                    ...
                    if r.lower() == "i":
                        if insert_station(cnn, network, station):
                            try_insert_files(
                                cnn, archive, station, network, StationCode, rnx_files
                            )
                    else:
                    ...
                    tqdm.write(
                        " -- External station %s.%s not found. Possible matches are %s"
                        % (
                            NetworkCode,
                            StationCode,
                            ", ".join(
                    ...
                    options = ", ".join(
                        [
                            "%s.%s (%i)" % (m["NetworkCode"], m["StationCode"], i + 1)
                    ...
                    r = input(
                        "\n  Insert new station %s with network code %s "
                        "or add this data as %s: (i)nsert new/(number)"
                        % (StationCode, network, options)
                    )
                    ...
                    tqdm.write(
                        " -- External station %s.%s not found. Closest station is %s.%s: %.3f m"
                        % (
                            NetworkCode,
                            StationCode,
                    ...
                    # ask the user what to do with the data
                    r = input(
                        "\n  Insert new station with default station network %s?: y/n "
                        % network
                    )
...
def insert_station(cnn, network, station):
    ...
    rstn = cnn.query_float(
        "SELECT * FROM stations WHERE \"NetworkCode\" = '%s' AND "
        "\"StationCode\" = '%s'" % (network, station["StationCode"])
    )
    if len(rstn) > 0:
        tqdm.write(
            " -- Station code %s already exists in network %s. Cannot insert station"
        ...
        if not cnn.query_float(
            "SELECT * FROM networks WHERE \"NetworkCode\" = '%s'" % network
        ):
            ...
            try:
                with pyRinex.ReadRinex(NetworkCode, StationCode, rnx) as rinexinfo:
                    ...
                    if not inserted:
                        ...
                        tqdm.write(
                            " -- %s.%s (%s) not imported: already existed in database."
                            % (NetworkCode, StationCode, os.path.basename(rnx))
                        )
                    else:
                        tqdm.write(
                            " -- %s.%s (%s) successfully imported into database."
                            % (NetworkCode, StationCode, os.path.basename(rnx))
                        )
                    ...
                    try:
                        ...
                        for record in import_stninfo:
                            import_record = StationInfoRecord(
                                NetworkCode, StationCode, _record=record
                            )
                            ...
                            if (
                                rinexinfo.datetime_firstObs
                                >= import_record.DateStart.datetime()
                                and not import_record.DateEnd.datetime()
                                <= rinexinfo.datetime_firstObs
                            ):
                                if (
                                    rinexinfo.datetime_lastObs
                                    > import_record.DateEnd.datetime()
                                ):
                                    tqdm.write(
                                        "    WARNING! RINEX file %s has an end data past the station info "
                        ...
                        if not stninfo_inserted:
                            tqdm.write(
                                "    Could not find a valid station info in the database or in the station "
                                "package. File remains in database without metadata."
                            )
            except pyRinex.pyRinexException as e:
                tqdm.write(
                    "    Problem while processing RINEX file for database record insert: "
                    + str(e)
                )
    else:
...
def get_rinex_file(cnn, stnlist, date, Archive_path):
    ...
    for stn in tqdm(stnlist, ncols=80, disable=None):
        ...
        if rinex is not None:
            ...
            tqdm.write(
                " -- %s not found for %s.%s"
                % (date.yyyyddd(), NetworkCode, StationCode)
            )
...
def main():
    ...
    parser.add_argument(
        "stnlist", type=str, nargs="+", metavar="all|net.stnm", help=station_list_help()
    )

    parser.add_argument(
        "-rinex",
        "--rinex",
        metavar="{ignore_stnlist}",
        type=int,
        nargs=1,
        default=None,
    ...
    if args.rehash is not None:
        ...
        hash_check(
            ...
            dates[1],
            rehash=True,
            h_tolerant=args.stninfo_tolerant[0],
        )
    ...
    if args.export_station is not None:
        export_station(
            cnn, stnlist, pyArchive, Config.archive_path, args.export_station
        )
