...
from geode import Utils, dbConnection
...
from geode.etm.core.type_declarations import JumpType, SolutionType
...
from geode.Utils import add_version_argument, process_date, station_list_help
...
""",
    ...
    parser.add_argument(
        "stations", type=str, nargs="+", metavar="station", help=station_list_help()
    )
    ...
    jump_parser = subparsers.add_parser(
        "jump", aliases=["j"], help="Add or remove discontinuities/jumps"
    )
    jump_action = jump_parser.add_mutually_exclusive_group(required=True)
    jump_action.add_argument("--add", "-a", action="store_true", help="Add a new jump")
    jump_action.add_argument(
        "--remove", "-r", action="store_true", help="Remove an existing jump"
    )
    ...
    copy_action.add_argument(
        "--status", "-s", action="store_true", help="Show current copy-params status"
    )
    ...
    print_parser = subparsers.add_parser(
        "print", aliases=["show", "ls"], help="Display current ETM parameters"
    )
    ...
    reset_parser.add_argument(
        "--polynomial", action="store_true", help="Reset polynomial parameters"
    )
    reset_parser.add_argument(
        "--periodic", action="store_true", help="Reset periodic parameters"
    )
    reset_parser.add_argument(
        "--jumps", action="store_true", help="Reset all jump parameters"
    )
    reset_parser.add_argument("--all", action="store_true", help="Reset all parameters")
    add_solution_args(reset_parser)

    # === BULK-REMOVE command ===
    bulk_parser = subparsers.add_parser(
        "bulk-remove", aliases=["bulk"], help="Bulk remove jumps by criteria"
    )
    bulk_type = bulk_parser.add_mutually_exclusive_group(required=True)
    bulk_type.add_argument(
        "--earthquakes",
        "-e",
        type=float,
        metavar="MAX_MAG",
    ...
    bulk_type.add_argument(
        "--mechanical", "-m", action="store_true", help="Remove mechanical jumps"
    )
...
def create_etm_params(
    cnn, network_code: str, station_code: str, soln: str
) -> EtmParams:
...
def create_etm_engine(
    cnn, network_code: str, station_code: str, soln: str, stack_name: str = None
) -> EtmEngine:
    """Create an EtmEngine instance for a station/solution combination."""
    solution_type = SolutionType.from_code(soln)
    solution_options = SolutionOptions(
        solution_type=solution_type, stack_name=stack_name or soln
    )
    config = EtmConfig(
        network_code=network_code,
        station_code=station_code,
...
def handle_polynomial(cnn, stnlist: List[Dict], args):
    ...
    params = {"object": "polynomial", "terms": args.terms}
    ...
    for station in stnlist:
        for soln in get_solution_types(args):
            etm_params = create_etm_params(
                cnn, station["NetworkCode"], station["StationCode"], soln
            )
            print(
                f"  Setting polynomial terms={args.terms} for "
...
def handle_periodic(cnn, stnlist: List[Dict], args):
    ...
    for station in stnlist:
        for soln in get_solution_types(args):
            ...
            print(
                f"  Setting periodic periods=[{periods_str}] days for "
                f"{station['NetworkCode']}.{station['StationCode']} ({soln})"
            )
            etm_params.push_params(params=params)
...
def handle_jump(cnn, stnlist: List[Dict], args):
    ...
    for station in stnlist:
        for soln in get_solution_types(args):
            ...
            print(
                f"  {action_str} {args.type} jump on {date.year}/{date.doy:03d} for "
                f"{station['NetworkCode']}.{station['StationCode']} ({soln})"
            )
            etm_params.push_params(params=params)
...
def handle_copy_params(cnn, stnlist: List[Dict], args):
    ...
    if args.enable:
        ...
        for station in stnlist:
            ...
            etm_params = create_etm_params(
                cnn, station["NetworkCode"], station["StationCode"], args.source
            )

            print(
                f"  Enabling copy-params for {station['NetworkCode']}.{station['StationCode']} "
        ...
        for station in stnlist:
            ...
            etm_params = create_etm_params(
                cnn, station["NetworkCode"], station["StationCode"], "gamit"
            )
            print(
                f"  Disabling copy-params for {station['NetworkCode']}.{station['StationCode']}"
            )
        ...
        for station in stnlist:
            ...
            print(
                f"  {station['NetworkCode']}.{station['StationCode']}: copy-params is {status}"
            )
...
def handle_print(cnn, stnlist: List[Dict], args):
    ...
    if args.etm:
        if "gamit" in solutions and not args.stack:
            raise EtmParamsException(
                "--stack is required when using --etm with gamit solutions"
            )
    ...
    for station in stnlist:
        ...
        if etm_params.is_copy_params_enabled():
            print("  [copy-params: enabled]")
        else:
            print("  [copy-params: disabled]")
...
def print_db_params(cnn, station: Dict, soln: str):
    ...
    if params["periodic"]:
        ...
        for period, status in params["periodic"].items():
            status_str = (
                status.description if hasattr(status, "description") else str(status)
            )
...
def print_etm_params(cnn, station: Dict, soln: str, args):
    ...
    try:
        ...
        etm = create_etm_engine(
            cnn, station["NetworkCode"], station["StationCode"], soln, stack_name
        )
        ...
        if params["jumps"]:
            print(
                f"      {'Date':<12} {'Type':<25} {'Relaxation':<20} {'Fit':<6} {'Metadata':<30}"
            )
            ...
            for jump in params["jumps"]:
                ...
                relax_str = (
                    str(jump["relaxation"])
                    if jump["relaxation"] and any(r > 0 for r in jump["relaxation"])
                    else "-"
                )
                fit_str = "yes" if jump["fit"] else "no"
                meta_str = (jump.get("metadata", "-") or "-")[:30]
                print(
                    f"      {date_str:<12} {jtype:<25} {relax_str:<20} {fit_str:<6} {meta_str:<30}"
                )
        else:
            print("      (none)")
...
def handle_reset(cnn, stnlist: List[Dict], args):
    ...
    if not any([args.polynomial, args.periodic, args.jumps, args.all]):
        raise EtmParamsException(
            "Specify what to reset: --polynomial, --periodic, --jumps, or --all"
        )
    ...
    for station in stnlist:
        for soln in get_solution_types(args):
            ...
            print(
                f"  Resetting {', '.join(what)} for "
                f"{station['NetworkCode']}.{station['StationCode']} ({soln})"
            )
            etm_params.push_params(
                reset_polynomial=reset_poly,
        ...
        for soln in solutions:
            ...
            try:
                ...
                if soln == "gamit" and not args.stack:
                    print("    Warning: --stack required for GAMIT solutions, skipping")
                    continue
            ...
            if args.earthquakes is not None:
                ...
                for jump in etm.jump_manager.jumps:
                    if (
                        jump.p.jump_type in earthquake_types
                        and jump.magnitude <= args.earthquakes
                    ):
                        if start_date <= jump.date <= end_date:
                            ...
                            removed_count += 1
                            print(
                                f"    Removed M{jump.magnitude:.1f} earthquake on "
                ...
                for jump in etm.jump_manager.jumps:
                    if jump.p.jump_type in mechanical_types:
                        if start_date <= jump.date <= end_date:
                            ...
                            removed_count += 1
                            print(
                                f"    Removed mechanical jump on "
                                f"{jump.date.year}/{jump.date.doy:03d}"
                            )

            if removed_count == 0:
                print("    No matching jumps found")
            else:
